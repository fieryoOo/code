...
c ==========================================================
      subroutine dmultifft(len, amp, phase, rec_am, lag, seis_out,ns)
      implicit none
      integer*4 len,ns
      real*4    amp(len),phase(len),rec_am(len),rec_am1(400000)
      include 'fftw3.h'
      integer*4 lag, i,k
c      real*4    cor(10001)
      real*8    plan3, fn
...
c multiply to get correlation in freq domain
      do i =1,len
          ...
          temp3(i) = dcmplx(real(dreal(temp3(i)))*rec_am1(i)*rec_am(i),
     1                      dimag(temp3(i)))
      enddo
