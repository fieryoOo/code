...
void dcommon_(int *len, float *amp,float *phase, float *rec_am);
void dmultifft_(int *len,float *amp,float *phase,float *rec_am, int *lag,float *seis_out, int *ns);
...
/*--------------------------------------------------------------------------*/
{
  ...
  int rec_b1[1000],rec_e1[1000],rec_b2[1000],rec_e2[1000],day_num[sdb->nst][sdb->nst];
  char filename[200], filename_temp[200], rec_name[200], rec_sac[200], amp_sac[200], phase_sac[200], filename_tp[100],filename_tp0[100], str[600];
  float amp[400000], phase[400000], rec_am[400000], cor_temp[40000], cor[40000],sactemp[40000];
  ...
  // outermost loop over day number, then station number
    ...
    for( jsta1 = 0; jsta1 < sdb->nst; jsta1++ ) {  
      ...
      if(sdb->rec[ine][jsta1].n > 0){
        ...
        sprintf( rec_sac, "%s.recam", sdb->rec[ine][jsta1].ft_fname );
// read amp and phase files and read into common memory
	  ...
	  fprintf(stderr,"%s is not a valid file\n",amp_sac);
        ...
        if ( read_sac(rec_sac, rec_am, &shdamp1, 1000000 )==NULL ) {
          fprintf( stderr,"file %s did not found\n", rec_sac );
          goto loop1;
        }
        if ( isnan(rec_am[0]) ) {
	...
	len = shdamp1.npts;
        ...
        dcommon_( &len, amp, phase, rec_am ); // reads amp and phase files into common memory
     ...
     for( jsta2 = (jsta1+1); jsta2 < sdb->nst; jsta2++ ) {
	    ...
	    sprintf( rec_sac, "%s.recam", sdb->rec[ine][jsta2].ft_fname );
	  fprintf(stderr,"Correlating: %s  %s\n", sdb->rec[ine][jsta1].ft_fname,sdb->rec[ine][jsta2].ft_fname );
	    ...
	    if ( read_sac(rec_sac, rec_am, &shdamp2, 1000000 )==NULL ) {
              fprintf( stderr,"file %s did not found\n", rec_sac );
              goto loop1;
            }
            if ( isnan(rec_am[0]) ) {
	    ...
	    len = shdamp2.npts;
            ...
            else
            {
                 ...
                 dmultifft_(&len, amp, phase, rec_am, &lag, seis_out,&ns);
...
//  for ( ns2 = 1; ns2 < sdb.nst; ns2++ ) for ( ns1 = 0; ns1 < ns2; ns1++ ) {
      sprintf(filename, "COR/%s/COR_%s_%s.SAC.prelim", sdb.st[ns1].name, sdb.st[ns1].name, sdb.st[ns2].name);
      sprintf(str, "mv %s COR/%s/COR_%s_%s.SAC",
      filename, sdb.st[ns1].name, sdb.st[ns1].name, sdb.st[ns2].name);
