...
char fname[300];
/*--------------------------------------------------------------*/
int get_snr(char *fname, int *flag)
/*--------------------------------------------------------------*/
{
  FILE *fp1, *f2;
  ...
  if(flag==1) {
     ii=(n-1)/2;
     for(k=1;k<=ii;k++) sig0[ii+k]=(sig0[ii+k]+sig0[ii-k])/2.;
     for(k=0;k<=ii;k++) { sig0[k]=sig0[k+ii]; sig0[k+ii]=0; }
     n=ii+1;
    }
...
/*--------------------------------------------------------------*/
{
  ...
  if(argc != 3) 
    {
      printf("Usage: spec_snr_rms list_file file_type[0 for single-sided, 1 for double-sided]\n");
      exit(-1);
    }
  if( atoi(argv[2])!=0 && atoi(argv[2])!=1 ) {
     printf("Input 0 or 1 for file_type!\n");
     exit(1);
    }
