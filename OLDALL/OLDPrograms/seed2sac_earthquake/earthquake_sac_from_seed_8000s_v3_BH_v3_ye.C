...
// two files are requred to run this program: station.lst and input_ev_seed.
#define MAIN
...
{
  int ist, iev;
  char data_path[]="/media/WORK/tianye/Wells_6/SEED/";
	  ...
	  for ( ist = 0; ist < sdb_E.nst; ist++ ) 
	    {
	      ...
	      //              mk_one_rec (&sdb, iev-1, ist, buff, "BHE");
	      // mk_one_rec (&sdb, iev-1, ist, buff, "BHZ");
	    }
	  system("/bin/rm *SAC");
	  system("/bin/rm RESP*");
