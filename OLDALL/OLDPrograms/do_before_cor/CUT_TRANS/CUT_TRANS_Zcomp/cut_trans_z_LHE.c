...
#include <stdlib.h>
#include <math.h>
#include "/home/yingjie/progs/NOISE_CODA/mysac.h"
#include "/home/yingjie/progs/NOISE_CODA/sac_db.h"
...
/*--------------------------------------------------------------------------*/
{
  ...
  if ( !read_sac ("s1_LHE.sac", sig1, &shd1, 1000000 ) ) {
    fprintf(stderr,"file %s not found\n", sd->rec[ne][ns].fname );
    return;
  }
...
/* ASSUME THAT THE DATA ARE WITHIN THE FOLLOWING FILTER BAND */
  ...
  sprintf(str,"/bin/cp %s s1_LHE.sac\0", sd->rec[ne][ns].fname ); 
  system(str);

/* GENERATE TEMPORARY FILES */
  sprintf(str,"/bin/cp %s resp1_LHE\0", sd->rec[ne][ns].resp_fname );
  system(str);

  ff = fopen("sac_bp_respcor_LHE","w");
  fprintf(ff,"/home/nshapiro/PROGS/SAC/bin/sac2000 << END\n");
  fprintf(ff,"r s1_LHE.sac\n");
  fprintf(ff,"rmean\n");
  fprintf(ff,"rtrend\n");
  fprintf(ff,"transfer from evalresp fname resp1_LHE to vel freqlimits %f %f %f %f\n", fl1, fl2, fl3, fl4 );
  fprintf(ff,"w s1_LHE.sac\n");
  fprintf(ff,"quit\n");
  fprintf(ff,"END\n");
  fclose(ff);

  system("sh sac_bp_respcor_LHE");
  system("/bin/rm resp1_LHE");
}
...
/* OPEN SAC DATABASE FILE AND READ IN TO MEMORY */
  if((ff = fopen("sac_db_LHE.out", "rb"))==NULL) {
    fprintf(stderr,"sac_db_LHE.out file not found\n");
    exit(1);
  }
