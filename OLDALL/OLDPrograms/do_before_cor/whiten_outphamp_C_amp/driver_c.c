...
static float seis_outamp[Nf],seis_outph[Nf],rec_am[Nf];
...
//printf("Check !!! %f %f %f %f %d %f %d\n",f1,f2,f3,f4,npow,dt,n);
  ...
  filter4_(&f1,&f2,&f3,&f4,&npow,&dt,&n,seis_in,rec_am,seis_outamp,seis_outph,&ns,&dom);
	shd1.npts = n;
	shd1.delta = dt;
//        write_sac(name,seis_out, &shd1);
	...
	shd1.b = 0;
        ...
        strcpy(nameamp,name);
        strcat(nameamp,".recam");
        write_sac( nameamp, rec_am, &shd1 );
