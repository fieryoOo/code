...
c=s==============================================================
      ...
      real*8    sorig(400000), senv(20000), curv(20000), sout(400000), dom, avg, avg2, cavg
      real*8    f, sum, fb, fe, f20, f30
c ---
      do i = 1,nk
        f = (i-1)*dom
	 ...
	 enddo
      ...
      cavg = 0
      count = 0
      do i = 3,int((f20-fb+1)/20)-2
	 curv(i)=((senv(i+2)+senv(i-2))/12.-4*(senv(i+1)+senv(i-1))/3.+5*senv(i)/2)
         if( curv(i).gt.0 ) then
            cavg = cavg + curv(i)
            count = count + 1
         endif
c         if( curv(i).gt.20 )write(*,*) (i*20-10+fb)*dom, curv(i)
      enddo
      curv(2) = 0
...
c      write (*,*) cavg
	...
	count = 0
	do i = 5,int((f20-fb+1)/20)-4
	    if( curv(i).lt.cavg*4 ) then
               goto 10
            endif
	    avg = (senv(i-2)+senv(i-3)+senv(i-4))/3.
c            write (*,*) (i*20-10+fb)*dom, avg*1.6, senv(i)
	    if(senv(i).lt.(avg*1.6)) goto 10
	    count = count + 1
