...
--------------------------------------------------------------------------*/
{
  ...
  for ( i = 1901; i < yy; i++ )
    {
      if ( (i%4 == 0 && i%100 != 0) || i%400==0 ) nyday += 366;
      else nyday += 365;
    }
...
double t1[200000], t2[200000], T1, T2;
SAC_HD sd[200000], s0;
int nf;
...
/*------------------------------------------------------------------------*/
{
  ...
  fprintf (ff, "/home/weisen/bin/rdseed <<END\n");
  fprintf (ff, "%s", nseed);
  fprintf(ff,"\n");                             /* out file */
  fprintf(ff,"\n");                             /* volume */
  fprintf(ff,"d\n");                            /* option */
...
/*---------------------------over----------------------------------*/
  ...
  /*---------- response file -----------*/
  sprintf(str,"ls SAC_PZs_*%s*%s* > list_resp\0",  sdb->st[ns].name,  ch);
  system(str);
  ...
  if ( fscanf(ff,"%s", resp_name ) != EOF ) {
     sprintf(str,"/bin/rm %s\0", resp_name);
     system(str);
    }
  fclose(ff);
  ...
  /*------------- mooving sac file -------*/
  sprintf(str,"/bin/mv merged.sac %s/%s.%s.%s.SAC\0", sdb->ev[ne].name, sdb->ev[ne].name, sdb->st[ns].name, ch);
  system(str);
  
  sprintf(sdb->rec[ne][ns].fname,"%s/%s.%s.%s.SAC\0", sdb->ev[ne].name, sdb->ev[ne].name, sdb->st[ns].name, ch);
  sprintf(sdb->rec[ne][ns].ft_fname,"%s/ft_%s.%s.%s.SAC\0", sdb->ev[ne].name, sdb->ev[ne].name, sdb->st[ns].name, ch);
...
/*------------------------------------------------------------------------*/
{
  ...
  char channel[3];
  if ( argc < 2 ) {
    fprintf(stderr, "usage: sac_from_seed LHZ\n");
    return 1;
  }
  ...
  fprintf(stderr,"sdb.nst: %d  sdb.nev: %d", sdb.nst, sdb.nev);
//  abort();
