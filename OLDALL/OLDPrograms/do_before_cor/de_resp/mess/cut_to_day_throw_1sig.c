...
void one_rec_cut(SAC_DB *sd, int ne, int ns, float t1, float n, char *DATAROOT);
...
/*--------------------------------------------------------------------------*/
{
  float sig1[1000000],sig2[1000000],sig3[1000000],sig4[1000000];
 ...
 if ( sd->rec[ne][ns].n <= 0 ) return;
  ...
  fprintf(stderr,"t1 %lg  t2 %lg   t1b %lg  t1e %lg\n", t1, t2, t1b, t1e);
    ...
    if ( !read_sac ("s1.sac", sig1, &shd2, 1000000 ) ) {
       fprintf(stderr,"file %s not found\n", sd->rec[ne][ns].fname );
       return;
    }

    if ( !read_sac ("s1.sac", sig1, &shd3, 1000000 ) ) {
       fprintf(stderr,"file %s not found\n", sd->rec[ne][ns].fname );
       return;
	}
...
//sig4 contain only the real signals, they are attached together without any manmade sig, but the time of the sig4 is not true
...
/*--------------------------------------------------------------------------*/
{
  ...
  static SAC_DB sdb;
	char DATAROOT[300];
 /*this program is aimed to modify those */	
/* CHECK INPUT ARGUMENTS */
  if ( argc < 4 ) {
    fprintf(stderr,"USAGE: cut_trans [t1] [npts] [DATAROOT]\n");
    exit(1);
  }
/*  t1=atof(argv[1]);
  npts=atof(argv[2]); */
...
/* OPEN SAC DATABASE FILE AND READ IN TO MEMORY */
  if((ff = fopen("sac_db_throw1.out", "rb"))==NULL) {
    fprintf(stderr,"sac_db_throw1.out file not found\n");
    exit(1);
  }
...
/* REMOVE INSTRUMENT RESPONSE AND CUT TO DESIRED LENGTH */
  ...
  //  one_rec_trans( &sdb, ne, ns);
   // fprintf(stderr,"back to main prog\n");
    ...
    one_rec_cut( &sdb, ne, ns, t1, npts,DATAROOT);
