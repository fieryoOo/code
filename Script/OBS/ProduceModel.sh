...
Initialize() {
   stalst=station.lst
   AgeMap=../Age/age.3.6.xyz.HD
   BathyMap=../Bathymetry/sea_floor.CICE.HD
   SedThk1Map=../Sediments/map1.thk1_JDF.HD
   SedThk2Map=../Sediments/map1.thk2_JDF.HD
   fCrust=Crust_Qmiu
   dirMantle=/mtera/tianye/ASN_OBS_BHZ/PaperResults/DISP_Pred/Mantle_Q_extrapolate
   dirQmiu=/mtera/tianye/ASN_OBS_BHZ/PaperResults/DISP_Pred/Qmiu_30
   ...
   PathAvgexe=/home/tianye/code/Programs/MapOperation/PathAvg/PathAvg
...
ComputeAll() {
   elamda=10
   age=`$PathAvgexe $AgeMap $slon $slat $rlon $rlat $elamda | awk '{printf "%.1f", $1}'`
   bathy=`$PathAvgexe $BathyMap $slon $slat $rlon $rlat $elamda | awk '{print -$1/1000.}'`
   sedthk1=`$PathAvgexe $SedThk1Map $slon $slat $rlon $rlat $elamda`
   sedthk2=`$PathAvgexe $SedThk2Map $slon $slat $rlon $rlat $elamda`
...
PredDisp() {
   ...
   /home/tianye/code/Programs/SURF_DISP/SURF_DISP model.tmp $1 R 0 1 $perl $perh 0.3 -a -f $cflag >& /dev/null
   ...
   if [ `echo $perc | awk '{if($1<0.8){print 1}else{print 0}}'` == 1 ]; then
      ...
      /home/tianye/code/Programs/SURF_DISP/SURF_DISP model.tmp $1 R 0 1 $perl2 $perh 0.3 -a -f $cflag >& /dev/null
   fi
}
