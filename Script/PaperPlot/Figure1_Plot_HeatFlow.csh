...
set cptfile = HeatFlow/JuanDeFuca_HeatFlow.txt.cpt
set cptfile2 = HeatFlow/JuanDeFuca_HeatFlow.txt.cpt
set input_map = HeatFlow/JuanDeFuca_HeatFlow.txt.HD
set res = 0.05
set region_f = area/region_OBS
set sta_lst=`echo $input_map | awk -F.HD '{print $1}'`
set sta_name=`echo $sta_lst"_name"`
...
### shift 1 ###
grdimage $SCA $REG -Ba2f0.5g1/a1f0.5g1WSen:."$title": tomo.grd -Itomo_gradient.grd -C$cptfile -X3 -Y5 -K >! $output_ps_file
...
psscale -B:"HeatFlow (mW/m^2)": -C$cptfile2 -D3.9/-1/7.8/0.3h -O -K  >> $output_ps_file

psxy $sta_lst $SCA $REG -Sc.05 -W0.1 -Gwhite  -O -K >> $output_ps_file
#awk '{printf "%f %f ",$1,$2}' $sta_name | psxy $REG $SCA -Svs0.03c/0.4c/0.15c -W4,red -Gred -O -K >> $output_ps_file
#awk '{print $1,$2}' $sta_name | psxy $REG $SCA -W6,red -Gred -O -K >> $output_ps_file
#psxy $sta_name $SCA $REG -St.3 -W3 -Gred  -O -K >> $output_ps_file
#pstext $sta_name $SCA $REG -S0.5p -O -K >> $output_ps_file
pstext -R0/10/0/10 $SCA -V -O -K -N << EOF >>  $output_ps_file
-0.5 8.5 20 0.0 4 LT (a)
EOF
exit
...
psbasemap $SCA2 $REG2 -G255 -B30/30wsen -O -K -X-0.3 -Y8 >> $output_ps_file
...
/home/tianye/code/Programs/pssac/pssac $REG -JX3.75i/.85i -B'a'$ymrk'f'$ytic':'"Time (sec)"':/a'$xmrk'f'$xtic':'::."Time Series":'WeSn' -W1 -X10.3 -Y-0.7 $sacf -K -O >> $output_ps_file
   ### negative 0.9 ~ 4.2
...
/home/tianye/code/Programs/pssac/pssac $REG -JX3.75i/.85i cut2 -W,blue -K -O >> $output_ps_file
pstext -R0/10/0/10 -JX3.5i/.8i -V -O -K -N << EOF >>  $output_ps_file
-0.5 16 20 0.0 4 LT (b)
EOF
rm -f cut1 cut2
...
### shift 3
grdimage $REG $SCA amp.grd -X0.5 -Y-8.2 -C'CC/amp.cpt' -O -K >> $output_ps_file
...
pstext -R0/10/0/10 $SCA -V -O -N << EOF >>  $output_ps_file
