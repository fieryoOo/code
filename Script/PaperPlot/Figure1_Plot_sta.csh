...
set cptfile2 = area/OBS_topo_rev.cpt
set input_map = area/OBS_topo.HD
set res = 0.01
set region_f = area/region_OBS
set sta_lst=`echo $input_map | awk -F.HD '{print $1}'`
set sta_name1=`echo $sta_lst"_name1"`
set sta_name2=`echo $sta_lst"_name2"`

set title='Study Area'
...
set sms=`echo $res | awk '{print $1/10.0}'`
...
### shift 0 ###
grdimage $SCA $REG -Ba2f0.5g1/a1f0.5g1WSen:."$title": tomo.grd -Itomo_gradient.grd -C$cptfile -X3 -P -Y14 -K >! $output_ps_file


#psxy /home/tianye/code/Programs/head/wus_province_II.dat $SCA $REG -W5/255/0/0 -M"99999 99999"  -O -K >> $output_ps_file
psxy /home/tianye/code/Programs/head/platebound.gmt $SCA $REG -W8/70/70/70 -M"99999 99999"  -O -K >> $output_ps_file
psscale -B1000:"Bathymetry (m)": -C$cptfile2 -D3.6/-1/7.2/0.25h -O -K  >> $output_ps_file

psxy $sta_name2 $SCA $REG -St.3 -W3 -G70/70/70  -O -K >> $output_ps_file
...
#awk '{print $1,$2}' $sta_name1 | psxy $REG $SCA -W6,red -Gred -O -K >> $output_ps_file
psxy $sta_name1 $SCA $REG -St.3 -W3 -Gblack  -O -K >> $output_ps_file
pstext $sta_name1 $SCA $REG -Gblack -O -K >> $output_ps_file
echo 230.0 46.06 | psxy $SCA $REG -Sa.3 -Gred -O -K >> $output_ps_file
echo 229.9 46.1 12 0 1 3 CHS | pstext $SCA $REG -Gred -O -K >> $output_ps_file
pstext -R0/10/0/10 $SCA -V -O -K -N << EOF >>  $output_ps_file
-1. 8.2 20 0.0 4 LT (a)
EOF

set REG2 = `more area/region_OBS2`
set SCA2 = -JN${long}/0.9i
#makecpt -Crelief -Z > topo.cpt
#xyz2grd OBS_topo2.cgi -Gtomo.grd -I1 -V $REG2
#grdimage $SCA2 $REG2 tomo.grd -Ctopo.cpt -O >> $output_ps_file
#gmtset FRAME_PEN 0.5p
psbasemap $SCA2 $REG2 -G255 -B30/30wsen -O -K -X-0.2 -Y7.8 >> $output_ps_file
pscoast $SCA2 $REG2 -A1000 -Na -O -K -W1 >> $output_ps_file
echo $long1 $lat1"\n"$long1 $lat2"\n"$long2 $lat2"\n"$long2 $lat1"\n"$long1 $lat1 | psxy -JN -R -W3,red -O -K >> $output_ps_file

rm -f tomo.grd tomo2.grd tomo_gradient.grd

#echo -0.5 1.8 20 0.0 4 LT Figure 1 | pstext -R0/1/0/1 $SCA2 -V -O -K -N >>  $output_ps_file
...
#grdgradient tomo.grd -A0 -N2 -Gtomo_gradient.grd
### shift 1 ###
grdimage $SCA $REG -Ba2f0.5g1/a1f0.5g1WSen:."$title": tomo.grd -C$cptfile -X10 -Y-6.3 -O -K >> $output_ps_file #-Itomo_gradient.grd
...
psscale -B1.5:"Lithospheric Age (Ma)": -C$cptfile -D3./-1/6./0.3h -O -K  >> $output_ps_file
...
/home/tianye/code/Programs/pssac/pssac $REG $SCA -B'a'$ymrk'f'$ytic':'"Time (sec)"':/a'$xmrk'f'$xtic':'::."Time Series":'WeSn' -W1 -X-10.05 -Y-7.5 $sacf -K -O >> $output_ps_file
   ### negative 0.9 ~ 4.2
...
quit
END
/home/tianye/code/Programs/pssac/pssac $REG -J cut1 -W,red -K -O >> $output_ps_file
/home/tianye/code/Programs/pssac/pssac $REG -J cut2 -W0/80/255 -K -O >> $output_ps_file
pstext -R0/10/0/10 -JX3.5i/.8i -V -O -K -N << EOF >>  $output_ps_file
-0.5 17. 20 0.0 4 LT (c)
EOF
rm -f cut1 cut2
...
### shift 3
grdimage $REG $SCA amp.grd -X9.7 -Y-.8 -C'CC/amp.cpt' -O -K >> $output_ps_file
