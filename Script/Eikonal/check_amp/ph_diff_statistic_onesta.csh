...
foreach station (`awk '{if(NR>9){print}}' $argv[2] | sed s/' '/'@'/g | sed s/'\+'/''/g`)
...
if ( ! `awk -v long=$long -v lati=$lati '(long-$1)^2+(lati-$2)^2<0.00001' $argv[1]/$period'sec_10snr_'$dis'dis'/$event'.ph.txt_v2' | wc -l` )then
...
/home/tianye/code/Script/GMT/C_plot_travel throw_v2_onesta_temp temp_onesta.region

set ph_sta2=`awk -v long=$long -v lati=$lati '(long-$1)^2+(lati-$2)^2<0.00001 {print $3}' $argv[1]/$period'sec_10snr_'$dis'dis'/$event'.ph.txt_v2'`
