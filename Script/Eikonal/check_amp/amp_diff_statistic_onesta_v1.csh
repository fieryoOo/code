...
mkdir -p $argv[1]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v1'
rm -f $argv[1]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v1'/$sta_name
rm -f $argv[1]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v1'/$sta_name'_pctg'
foreach event (`more ../event_TA_XR.lst`)
#set event=20080819163018
if ( ! -e $event'_am.txt_v1' ) then
continue
endif
if ( ! `awk -v long=$long -v lati=$lati '(long-$1)^2+(lati-$2)^2<0.00001' $argv[1]/$period'sec_10snr_'$dis'dis'/$event'_am.txt_v1' | wc -l` )then
continue
endif

awk -v long=$long -v lati=$lati 'sqrt(1.0000*(($1-long)^2+($2-lati)^2))>0.44 && sqrt(1.0000*(($1-long)^2+($2-lati)^2))<2.26' $event'_am.txt_v1' > throw_v1_onesta_am_temp
if ( `more throw_v1_onesta_am_temp | wc -l` < 10 )continue
...
/home/tianye/code/Script/GMT/C_plot_travel throw_v1_onesta_am_temp temp_onesta_am.region

set amp_sta2=`awk -v long=$long -v lati=$lati '(long-$1)^2+(lati-$2)^2<0.00001 {print $3}' $argv[1]/$period'sec_10snr_'$dis'dis'/$event'_am.txt_v1'`
set lowlow=`grep $longlow'	'$latilow'	' throw_v1_onesta_am_temp.HD | awk '{print $3}'`
set lowhigh=`grep $longlow'	'$latihigh'	' throw_v1_onesta_am_temp.HD | awk '{print $3}'`
set highlow=`grep $longhigh'	'$latilow'	' throw_v1_onesta_am_temp.HD | awk '{print $3}'`
set highhigh=`grep $longhigh'	'$latihigh'	' throw_v1_onesta_am_temp.HD | awk '{print $3}'`
...
echo $amp_diff >> $argv[1]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v1'/$sta_name
echo $diff_pctg >> $argv[1]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v1'/$sta_name'_pctg'
#exit
end
#exit
cd ..
