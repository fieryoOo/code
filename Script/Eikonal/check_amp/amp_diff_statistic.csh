...
foreach station (`awk '{if(NR>13){print}}' $argv[3] | sed s/' '/'@'/g | sed s/'\+'/''/g`)
...
mkdir -p $argv[2]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v2'
rm -f $argv[2]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v2'/$sta_name
rm -f $argv[2]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v2'/$sta_name'_pctg'
foreach event (`more ../event_TA_XR.lst`)
#set event=20080819163018
if ( ! -e $argv[2]'/'$period'sec_10snr_'$dis'dis/'$event'_am.txt_v2' ) then
continue
endif
if ( ! -e $event'_am.txt_v2' ) then
continue
endif
if ( ! `awk -v long=$long -v lati=$lati '(long-$1)^2+(lati-$2)^2<0.00001' $argv[2]/$period'sec_10snr_'$dis'dis'/$event'_am.txt_v2' | wc -l` )then
continue
endif
awk -v long=$long -v lati=$lati 'sqrt(1.0000*(($1-long)^2+($2-lati)^2))>0.44 && sqrt(1.0000*(($1-long)^2+($2-lati)^2))<2.26' $event'_am.txt_v2' > throw_v2_am_temp
if ( `more throw_v2_am_temp | wc -l` < 10 )continue
...
/home/tianye/code/Script/GMT/C_plot_travel throw_v2_am_temp temp_am.region

set lowlow=`grep $longlow'	'$latilow'	' throw_v2_am_temp.HD | awk '{print $3}'`
set lowhigh=`grep $longlow'	'$latihigh'	' throw_v2_am_temp.HD | awk '{print $3}'`
set highlow=`grep $longhigh'	'$latilow'	' throw_v2_am_temp.HD | awk '{print $3}'`
set highhigh=`grep $longhigh'	'$latihigh'	' throw_v2_am_temp.HD | awk '{print $3}'`
...
#echo $gplong,$gplati
#echo $amp_pdt
set amp_sta2=`awk -v long=$long -v lati=$lati '(long-$1)^2+(lati-$2)^2<0.00001 {print $3}' $argv[2]/$period'sec_10snr_'$dis'dis'/$event'_am.txt_v2'`
set amp_diff=`echo "$amp_sta2-($amp_pdt)" | bc`
set diff_pctg=`echo "$amp_diff*1000000/$amp_pdt*0.0001" | bc`
echo $amp_diff >> $argv[2]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v2'/$sta_name
echo $diff_pctg >> $argv[2]/$period'sec_10snr_'$dis'dis'/amp_diff_$period's_v2'/$sta_name'_pctg'
#exit
end
#exit
cd ..
