...
#include <iostream>
#include <cstdlib>
#include "/projects/yeti4009/code/Programs/head/mysac64.h"
#include "/projects/yeti4009/code/Programs/head/sac_db64_1ev.h"
using namespace std;
...
/*c/////////////////////////////////////////////////////////////////////////*/
/*--------------------------------------------------------------------------*/
	void sac_db_write_to_asc ( SAC_DB *sdb, const char *fname )
/*--------------------------------------------------------------------------*/
{
  int ie, is;
  FILE *fi, *ff;
...
/*-------------------------------------------------------------------------*/
 int merge_sac(const char *sta, const char *chan, double *t0, float *dt, int *nrec, EVENT *ev)
/*-------------------------------------------------------------------------*/
{
  FILE *fi;
...
/*////////////////////////////////////////////////////////////////////////*/
/*------------------------------------------------------------------------*/
 void mk_one_rec (SAC_DB *sdb, int ne, int ns, const char *nseed, const char *ch)
/*------------------------------------------------------------------------*/
{
...
//  sprintf(str,"cp /data/minos-2/linf/Earthquake/all_seed_files/all_BHZ_RESP/%d/%s/RESP*%s*%s*  resp1\0",sdb->ev[ne].yy, sdb->ev[ne].name,  sdb->st[ns].name,  ch);
  ...
  fprintf(ff,"/home/yeti4009/Softwares/sac/bin/sac<< END\n");
...
/*------------------------------------------------------------------------*/
{
  ...
  for (iev=0;;)
      ...
      if ( fscanf(fii,"%s", &buff ) != EOF )
	{
	  ...
	  fprintf(fff,"/home/yeti4009/usr/bin/rdseed <<END\n");
	  fprintf(fff,"%s\n", buff );
	  fprintf(fff,"\n");                             /* out file */
	  fprintf(fff,"\n");                             /* volume */
	  fprintf(fff,"d\n");                            /* option */
