...
int Whiten( double f1, double f2, double f3, double f4, double dt, int n, float hlen, float *seis_in, float *seissm, float *outam, float *outph, int *ns, double *dom);
...
int SpectralNorm(char *fname, float *sig, SAC_HD shd, int ithread) {
   ...
   flag_whiten = Whiten( f1, f2, f3, f4, dt, n, frechlen, sig, sigw, outam, outph, &ns, &dom);
...
void * NormEvent (void *tid) {
   ...
   for(;;) {
      ...
      nst = 0;
      reports[ithread].tail += sprintf(reports[ithread].tail, "### Preparing am and ph records for event %s: ", sdb->ev[iev].name);
