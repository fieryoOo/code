...
c           set f2 to be -1. for a low-pass filter
...
c ==========================================================
      ...
      recursivesubroutine filter4(f1,f2,f3,f4,npow,dt,n,seis_in,
     *                    seis_out)
      implicit none
      include 'fftw3.h'
      integer*4 npow,n
      real*8    f1,f2,f3,f4,dt
      real*4    seis_in(n),seis_out(n)
c ---
      integer*4 k,ns,nk
      real*8    plan1,plan2
      real*8    dom
...
c determin the power of FFT
      ns = 2**max0(int(dlog(dble(n))/dlog(2.0d0))+1,1)
      dom = 1.0d0/dt/ns
      do k = 1,ns
        s(k) = czero
      enddo
...
c   make tapering
      if(f2.eq.-1.) then
         call taperL(f3,f4,dom,nk,npow,sf)
      else
         call taperB(f1,f2,f3,f4,dom,nk,npow,sf)
      endif
c===============================================================
...
c===============================================================
c Tapering subroutine (BandPass)
c===============================================================
      recursivesubroutine taperB(f1,f2,f3,f4,dom,nk,npow,sf)
...
c===============================================================
c Tapering subroutine (LowPass)
c===============================================================
      recursivesubroutine taperL(f3,f4,dom,nk,npow,sf)
      real*8    f3,f4,dom
      integer*4 nk,npow
      double complex sf(nk*2)
      real*8    d2,f,dpi,ss,s(nk*2)
      integer*4 i,j
c ---
      dpi = datan(1.0d0)*4.0d0
      do i = 1,nk
