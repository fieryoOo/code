...
c ==========================================================
      recursivesubroutine ifft(nlen,amp,pha,seis_out,ns)
      implicit none
      include 'omp_lib.h'
      integer*4 nlen, ns
c      integer, intent(in) :: nlen
      real*4    amp(nlen),pha(nlen)
...
c      common  /fftw3_mkl/ignore(4),number_of_user_threads
c      integer*4 :: ignore, number_of_user_threads
c      bind (c) :: /fftw3_mkl/
c      number_of_user_threads = 4

c read amp and phase into complex temp2
c multiply to get correlation in freq domain
 
      do i = 1,nlen
         temp3(i) = amp(i)*exp((0.0d0,-1.0d0)*pha(i))
      enddo
