...
char **Seed2Sac(int ne, int ns, char *nseed, int *nfile, int ithread) {
   ...
   /*---------mv sac files --------------*/
   sprintf(str, "mv *%s*%s*SAC %s", sdb->st[ns].name, ch, tdir);
   system(str);
   pthread_mutex_unlock(&rdslock); //unlock
   
   /*------------ sac list -------------*/
   sprintf(fname, "%s/list_sac", tdir);
   sprintf(str,"( ls %s/*%s*%s*SAC>%s ) >& /dev/null", tdir, sdb->st[ns].name, ch, fname);
   system(str);
...
int MakeRecord (int ne, int ns, int ithread) {
  ...
  SAC_HD sd[nfile];
  for(i=0;i<nfile;i++) {
     Resampling(filelst[i], &sigrspd[i], &sd[i], ithread);
  }

  i = merge_sac(sigrspd, sd, nfile, ne, ns, ithread);
  //cleanup
  system(str);
  if( ! i ) return 0;
...
void * ExtractEvent( void * tid ) {
   int ithread = *((int *)tid);
   int iev, ist, nst, flag;
  
   char tdir[100], str[300];
   sprintf(tdir, "Working_Thread_%d", ithread);
   sprintf(str, "mkdir -p %s", tdir);
   system(str);
   ...
   for(;;) {
      ...
      nst = 0;
      reports[ithread].tail += sprintf(reports[ithread].tail, "### Sac file extraction done for event %s: ", sdb->ev[iev].name);
      ...
      cout<<"Thread "<<ithread<<": "<<reports[ithread].head;
      reports[ithread].tail = reports[ithread].head;
   }

   sprintf(str, "rm -rf %s", tdir);
   system(str);
   pthread_exit(NULL);
}
...
void ExtractSac() {
   ...
   for(ithread=0;ithread<NTHRDS;ithread++) {
      ...
      rc = pthread_create( &tid[ithread], &attr_j, ExtractEvent, (void *) (&(targs[ithread])) );
      if(rc) {
         cout<<" Thread creation failed!  ERR: "<<strerror(rc)<<endl;
         exit(0);
      }
   }
