...
{ 
  ...
  int N=3000;
  FILE *ff,*f1,*f2;
  char file_name[400], stan[N][10], cbuff[300];
  double dis,dis_min;
  double per;
  per=atof(arg[2]);
  ...
  for(i=0;i<N;i++)
    {
      if(fscanf(f1,"%lf %lf %lf %lf %lf %s",&lon[i],&lat[i],&time[i],&vel[i],&amp[i],&stan[i])==EOF) 
	break;
      YesNo[i]=0;
    }
  ist=i;
  ...
  for(i=0;i<ist;i++)
    {
      //      fact[i]=lat[i]-lon[i];
      fact[i]=(lat[i]-41.4)*(lat[i]-41.4)+(lon[i]-245.1)*(lon[i]-245.1);
    }
  order[0]=0;
  for(i=1;i<ist;i++)
    {
...
printf("%s!\n",buff);
  ...
  for(i=0;i<ist;i++)
    {
      if(YesNo[order[i]]==0)
	fprintf(ff,"%lf %lf %lf %lf %lf %s\n",lon[order[i]],lat[order[i]],time[order[i]],vel[order[i]],amp[order[i]],stan[order[i]]);	
    }
