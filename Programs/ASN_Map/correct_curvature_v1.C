...
int main(int na, char *arg[])
{
  if(na!=6)
    {
      cout<<"usage:correct_tr_t_curvature [center station] [period] [least sta #] [greatest t curvature] [greatest am curvature]"<<endl;
      return 0;
    }
  FILE *ff,*fin,*fin_amp,*fin2,*fin3,*fout,*file1;
  int i,j;
  int npts_x,npts_y;
  char cbuff[300],buff1[300],sta1[10],path[300],statmp[10];
...
//  npts_y=61;
 ...
 y1=-999;
  ...
  sprintf(buff1,"%s_center_ph_map_v1.HD",arg[1]);
...
//  y1=y0+(npts_y-1)*dy;
  ...
  fin=fopen(buff1,"r");
  sprintf(buff1,"%s_center_am_map_v1.HD",arg[1]);
  ...
  marker_nn=0;
  int marker;
  sprintf(buff1,"%s_center_ph_amp_map_v1",arg[1]);
  ...
  ist=0;
  ist_old=0;
  fgets(cbuff, 300, fin3);
  for(;;)
    {
      if(fscanf(fin3,"%lf %lf %lf %lf %lf %s",&lon2,&lat2,&temp2,&temp3,&temp4,&statmp)==EOF) 
	{
	  fclose(fin3);
	  break;
	}
      ...
      if(temp2<2*period)
	{
	  ...
	  arr[ist][3]=temp3;
	  arr[ist][4]=temp4;
          sprintf(stan[ist],"%s",statmp);
       cout<<lon2<<" "<<lat2<<" "<<temp2<<" "<<temp3<<" "<<temp4<<" "<<statmp<<endl;
  ...
  if(ist*1.0/ist_old<0.4)
    {
      cout<<"too many stations removed!!"<<endl;
    }
  else
    if(ist<atof(arg[3]))
      {
	cout<<"too less station left!!"<<endl;
      ...
      {
	sprintf(buff1,"%s_center_ph_amp_map_v2",arg[1]);
	fout=fopen(buff1,"w");
        fprintf(fout,"%s",cbuff);
	for(i=0;i<ist;i++)
	  {
	    fprintf(fout,"%lf %lf %lf %lf %lf %s\n",arr[i][0],arr[i][1],arr[i][2],arr[i][3],arr[i][4],stan[i]); 
	  }
	fclose(fout);
      }
  return 0;
