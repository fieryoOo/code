...
#       sys.exit();
	...
	return(A0,A1,A2,fi1,fi2);

inb = [];
inv = [];
inu = []; 
outn = sys.argv[1] + ".4pi.fit"
outf = open(outn,"w");
...
(A0,A1,A2,fi1,fi2) = invert_1 ( inb,inv,inu);
fi1 = fi1*180./math.pi;
fi2 = fi2*180./math.pi;
print A0,A1,A2,fi1,fi2;
for i in range (361):
#	v0 = A0 + A1*math.sin(i + fi1) + A2*math.sin(2*i + fi2);
	v1 = A0;
	v2 = A1*math.sin(2*i*math.pi/180. + fi1*math.pi/180.);
	v3 = A2*math.sin(4*i*math.pi/180. + fi2*math.pi/180.);
	v0 =v1 + v2 + v3;
	outf.write("%g %g %g %g %g\n" % (i,v0,v1,v2,v3));
outf.close();
