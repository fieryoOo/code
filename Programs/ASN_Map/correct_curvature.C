...
int main(int na, char *arg[])
{
  if(na!=6)
    {
      cout<<"usage:correct_tr_t_curvature [rname] [period] [least sta #] [greatest t curvature] [greatest am curvature]"<<endl;
      return 0;
    }
...
//  npts_y=61;
 ...
 y1=-999;
  ...
  sprintf(buff1,"%s_ph_map_v1.HD",arg[1]);
...
//  y1=y0+(npts_y-1)*dy;
  ...
  fin=fopen(buff1,"r");
  sprintf(buff1,"%s_am_map_v1.HD",arg[1]);
  ...
  marker_nn=0;
  int marker;
  sprintf(buff1,"%s_ph_amp_map_v1",arg[1]);
  ...
  if(ist*1.0/ist_old<0.4)
	...
	cout<<"too less station left!!"<<endl;
      ...
      {
	sprintf(buff1,"%s_ph_amp_map_v2",arg[1]);
	fout=fopen(buff1,"w");
