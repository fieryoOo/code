...
c normalization amp diagram to 100 Db with three decade cutting
      ...
      do j = 1,ntall
        do k = 1,nf
          if(ampo(j,k).gt.amaxo) amaxo = ampo(j,k)
          if(amp(j,k).gt.amax) amax = amp(j,k)
       enddo
      enddo
      write(*,*) "amaxo: ",amaxo,"  amax: ",amax
      do j = 1,ntall
        do k = 1,nf
...
c             write(*,*) "2nd trigger:"
             ...
             iflag = 0
             do k=istrt,iend+1
                if(dabs(ftrig1(k)).ge.(tresh*1.5d0)) iflag = 1
             enddo
             if(iflag.eq.0) then
               do i =1,nf
                 grvelt(i) = grvel1(i)
        ...
        enddo
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c after removing possible jumps, cut frequency range to single
...
c        write(*,*) "3rd trigger:"
        ...
        if(ierr1.ne.0) then
          ...
          indx(1) = 1
          do i =1,nf
            if(dabs(ftrig1(i)).ge.(tresh*1.5d0)) then
              nindx = nindx+1
              indx(nindx) = i
            endif
          enddo
