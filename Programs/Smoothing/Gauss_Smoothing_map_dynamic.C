...
int gau_avg ( double *datax, double *datay, int idata, double alpha, double *avg )
{
   int i;
   double weit, weight=0.;
   *avg = 0;
   for(i=0;i<idata;i++) {
      weit=exp(-alpha*pow(datax[i],2));
      weight += weit;
      *avg = *avg + weit*datay[i];
   }
   *avg = *avg / weight;
   return 1;
}

int sort ( double *datax, double *datay1, int ndat )
{
  int i, j, ii;
...
int Gauss_Smoothing(char *fname, double hdis)
{
   ...
   double dis, itcpt, gsta, alpha, hdis1;
   ...
   int ndata[nsta];
   double dist[nsta][nsta], data[nsta][nsta], dsmd[nsta];

   for(i=0;i<nsta;i++){
      ndata[i]=0;
      for(j=0;j<nsta;j++){
         calc_dist(lat[i], lon[i], lat[j], lon[j], &dis);
         if(dis>4*hdis) continue;
         dist[i][ndata[i]]=dis; data[i][ndata[i]]=dat[j];
         ndata[i]++;
      }
   }
   ...
   gsta=0.; nesta=0;
   for(i=0;i<nsta;i++) {
      if(ndata[i]<3) continue;
      sort(&dist[i][0], &data[i][0], ndata[i]);
      for(j=0;j<ndata[i];j++) {
         if(dist[i][j]>2*hdis) break;
         gsta = gsta+1;
      }
      nesta++;
   }
   gsta = (int)(gsta/nesta+0.5);
   for(i=0;i<nsta;i++){
      j=(int)gsta;
      if(ndata[i]<j) j=ndata[i];
      hdis1 = dist[i][j-1]/2.;
      //cout<<"half dist for station "<<lon[i]<<" "<<lat[i]<<": "<<hdis1<<endl;
      alpha = 0.5/hdis1/hdis1;
      gau_avg(&dist[i][0], &data[i][0], j, alpha, &dsmd[i]);
   }

   sprintf(buff,"%s_smd_%.0f\0",fname,hdis);
   f1=fopen(buff,"w");
   for(i=0;i<nsta;i++)
      fprintf(f1,"%lf %lf %lf\n", lon[i], lat[i], dsmd[i]);
   fclose(f1);
