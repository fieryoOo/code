...
int Smoothing (char *infile, double hwidth) {
  ...
  double dx=hwidth*0.5,  alpha=0.5/(hwidth*hwidth);
  ...
  i = (int)ceil((datax[0]+hwidth)/dx); // min midx from datax[0]
  ...
  for(j=0; ;i++) {
     ...
     for(j=jstart;datax[j]<midx-hwidth2;j++);		// starting j for current midx
     jstart = j;					// next jstart cannot be smaller than current starting j
     for(j=jend; j<ndata; j++) { if( datax[j]>midx+hwidth2) break; }
