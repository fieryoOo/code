...
int main (int argc, char *argv[])
{
   ...
   cout<<"GEOL 6670   Homework set 3   Ye Tian"<<endl<<endl;
   double weit0[ndat];
   for(i=0;i<ndat;i++) weit0[i] = 1;
   least_fit_parabola (&datx[0], &daty[0], &weit0[0], ndat, &a, &b, &c, &std, &stdw);
   cout<<"non-weighted results:"<<endl<<endl;
   cout<<"1) "<<a<<" * x^2 + "<<b<<" * x + "<<c<<"  (See page 2 for plots)"<<endl<<endl;
   ...
   cout<<"weighted results:"<<endl<<endl;
   cout<<"3) "<<a<<" * x^2 + "<<b<<" * x + "<<c<<"  (See page 2 for plots)"<<endl;
   cout<<"   The weighted solution fits the 1st and 3rd measurements better, but not as good as the unweighted solution for the other 3 points."<<endl<<endl;
   cout<<"4) data variance (with denominator n-3): "<<std*std<<endl;
   cout<<"   data variance (weighted): "<<stdw*stdw<<endl;
   cout<<"   When computed with the same unweighted formula, the variance of the weighted solution is larger compared with the unweighted solution. This is obviously true because the weighted result is computed by minimizing a different(weighted) function. However when computed with the correct weighted formula, the variance of the weighted solution becomes smaller than the unweighted one. This actually means that the weighted solution fits the data better when we take the uncertanties into consideration."<<endl;
   return 1;
}
