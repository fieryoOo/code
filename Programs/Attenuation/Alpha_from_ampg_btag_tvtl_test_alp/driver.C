...
int main (int argc, char *argv[])
{
   ...
   double alpraw[NSTA][500], alpcrd[NSTA][500], alpsit[NSTA][500], alpprp[NSTA][500], azi[NSTA][500], dist[NSTA][500];
   char sta[NSTA][10];
   double lonmin=360, lonmax=0, latmax=-90, latmin=90;
   int i, j, iev, ilon, ilat, ist, nst, nsta, nstt, nbad, nalp[NSTA];
   double per=25., pi=3.1415926536, alpmn=-0.004, alpmx=0.0055;
 ...
 //for(i=0;i<nst;i++) cout<<lon[i]<<" "<<lat[i]<<"  beta: "<<beta[i]<<"  grdb: "<<grdb[i]<<"  azib: "<<azib[i]<<endl;
   ...
   for(iev=0;;iev++){
      ...
      for(i=0;i<nsta;i++){
         ...
         calc_dist(clati, clong, tlati[i], tlong[i], &dist[ist][nalp[ist]]);
         dec[i]=-(grda[i]*cos(azia[i]-azit[i]))/amp[i]-0.5/dist[ist][nalp[ist]];
         site[i]=2.*(grdb[ist]*cos(azib[ist]-azit[i]))/beta[ist];
         prp[i]=-1.*0.5/grdt[i]*lplc[i]+0.5/dist[ist][nalp[ist]];
...
*/
   ...
   for(i=0;i<nst;i++) {
      sprintf(buff,"test_alp_70/%.3f_%.3f_sit\0",lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpsit[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_70/%.3f_%.3f_prp\0",lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpprp[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_70/%.3f_%.3f_raw\0",lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpraw[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_70/%.3f_%.3f_crd\0",lon[i],lat[i]);
      fout=fopen(buff,"w");
...
/*
   ...
   for(i=0;i<nst;i++) fprintf(fout,"%lf %lf %lf %d\n",lon[i],lat[i],alpraw[i]/nalp[i], nalp[i]);
   fclose(fout);

   fout=fopen("alpha_crd_map","w");
   for(i=0;i<nst;i++) fprintf(fout,"%lf %lf %lf %d\n",lon[i],lat[i],alpcrd[i]/nalp[i], nalp[i]);
   fclose(fout);

   fout=fopen("alpha_site_map","w");
   for(i=0;i<nst;i++) fprintf(fout,"%lf %lf %lf %d\n",lon[i],lat[i],alpsit[i]/nalp[i], nalp[i]);
   fclose(fout);

   fout=fopen("alpha_prp_map","w");
   for(i=0;i<nst;i++) fprintf(fout,"%lf %lf %lf %d\n",lon[i],lat[i],alpprp[i]/nalp[i], nalp[i]);
   fclose(fout);
*/
   return 1;
}
