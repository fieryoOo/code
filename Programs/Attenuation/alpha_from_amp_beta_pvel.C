...
{
if (argc != 6) {
  cout<<"please input [amp_file] [beta_file] [pvel_file] [clong] [clati]"<<endl;
  return 0;
  }
   int i,j,nsta;
   int ilon, ilat;
   ...
   double lon[NSTA], lat[NSTA], amp[NSTA], clong, clati, lontmp, lattmp;
   double dattmp;
   ...
   double ampg[nptslon][nptslat], beta[nptslon][nptslat], pvel[nptslon][nptslat], ydata[nptslon][nptslat], azi[nptslon][nptslat], dist[nptslon][nptslat];

   for(i=0;i<nptslon;i++) for(j=0;j<nptslat;j++) {
      ampg[i][j] = -1; beta[i][j] = -1; pvel[i][j] = -1; }
   for(i=0;i<nptslon;i++) for(j=0;j<nptslat;j++) {
...
//      cout<<pvel[i][j]<<" "<<beta[i][j]<<" "<<log(1./beta[i][j]*100.*sqrt(1./pvel[i][j]*3.5))<<endl;
   ...
   double daty[5000], datx[5000], datyold[5000];
   double slp, itcpt, tazi, tdis, dismin, dismax;
   int iazi, idat;
   float bin=3.;

   for(iazi=0;iazi<360;iazi++){
//   for(iazi=90;iazi<91;iazi++){
      dismin=50000; dismax=0;
      for(i=0;i<nsta;i++) {
         calc_azimuth(clati,clong,lat[i],lon[i],&tazi);
         if( (tazi > iazi+bin && tazi < iazi-bin+360.) || (tazi < iazi-bin && tazi > iazi+bin-360.)) continue;
         tdis=get_dist(clati, clong, lat[i], lon[i]);
         if(dismin>tdis) dismin=tdis;
         if(dismax<tdis) dismax=tdis;
      }
      if(dismin<100) dismin = 100;
      idat=0;
      for(i=0;i<nptslon;i++) for(j=0;j<nptslat;j++) {
         if(dist[i][j]<dismin || dist[i][j]>dismax) continue;
         tazi=azi[i][j];
         if( (tazi > iazi+bin && tazi < iazi-bin+360.) || (tazi < iazi-bin && tazi > iazi+bin-360.)) continue;
         if(ydata[i][j]==-1) continue;
         datyold[idat] = ampg[i][j];
         daty[idat] = ydata[i][j];
         datx[idat] = dist[i][j];
        // cout<<idat<<": "<<datx[idat]<<" "<<dist[i][j]<<endl;
         idat++;
      }
      sprintf(buff,"int_alpha_r_curve_%ddeg\0", iazi);
      ouf=fopen(buff,"w");
      for(i=0;i<idat;i++) fprintf(ouf, "%.3f\t%g\t%g\n", datx[i], daty[i], datyold[i]);
      fclose(ouf);
