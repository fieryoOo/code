...
int calc_sms_grdt(char *file, double *slong, double *slati, double *sdat, double *grdt, double *azi, int *nsta)
{
   FILE *ff;
   ...
   double hdis=120.;
   Gauss_Smoothing(file, hdis);
   sprintf(buff,"%s_smd\0",file);
   ...
   for(ista=0;ista<*nsta;ista++){
      ...
      for(i=ii;i<ii+2;i++)
         for(j=jj;j<jj+2;j++){
            ...
            if(fabs((grdtmp-gradtr[i][j])/gradtr[i][j])>0.5) gradtr[i][j]=1e20;
...
{
   if(argc!=6){
      cout<<"usage:csta_map_gradient [input_trvt_map] [input_amp_map] [input_beta_map] [clong] [clati]"<<endl;
      return 0;
   }
   FILE *ff;
   char fname[100], *tmpc;
   double grdt[2000], azit[2000], lplc[2000], amp[2000], grda[2000], azia[2000], beta[2000], grdb[2000], azib[2000];
   double tlong[2000], tlati[2000], along[2000], alati[2000], dist[2000];
   double clong=atof(argv[4]), clati=atof(argv[5]);
   int i, nsta, nstt;

   if(clong<0) clong += 360.;
   calc_sms_grdt(argv[2], along, alati, amp, grda, azia, &nsta);
//for(i=1;i<nsta;i++) cout<<along[i]<<" "<<alati[i]<<" "<<amp[i]<<" "<<grda[i]<<" "<<azia[i]<<endl;
...
//      return 0;
//   }
   calc_grdt_beta(argv[3], along, alati, beta, grdb, azib, &nsta);
//   for(i=1;i<nsta;i++) cout<<along[i]<<" "<<alati[i]<<" "<<beta[i]<<" "<<grdb[i]<<" "<<azib[i]<<endl;

  for(i=0;i<nsta;i++) calc_dist(clati, clong, tlati[i], tlong[i], &dist[i]);

   double alpha[nsta], dec[nsta], site[nsta], prp[nsta];
   for(i=0;i<nsta;i++){
      if(azia[i]==-1||azit[i]==-1) continue;
      dec[i]=-(grda[i]*cos(azia[i]-azit[i]))/amp[i]-0.5/dist[i];
      site[i]=3.*(grdb[i]*cos(azib[i]-azit[i]))/beta[i];
      prp[i]=-1.*0.5/grdt[i]*lplc[i]+0.5/dist[i];
      alpha[i]=dec[i]+site[i]+prp[i];
      //if(alpha[i]>0.002 || alpha[i]<0.0002) azia[i]=-1;
//cout<<dec[i]<<": "<<endl;
//cout<<"2 * "<<grda[i]<<" * "<<grdt[i]<<" * cos("<<azia[i]<<" - "<<azit[i]<<") / "<<amp[i]<<endl;
   }
   ...
   for(i=0;i<nsta;i++){
      if(azia[i]==-1||azit[i]==-1||azib[i]==-1) continue;
//      fprintf(ff,"%8.4f  %8.4f  %8g\n", along[i], alati[i], -0.5/grdt[i]*lplc[i]);
