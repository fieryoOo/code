...
//*/
   ...
   //double slpmn=, slpmx=;
   int count, icoef;
   sprintf(buff,"mkdir -p test_alp_sm_%.0f_dynamic_2.0", hdis);
   system(buff);
   for(i=0;i<nst;i++) {
      fct=1e9;
      ...
      sprintf(buff,"test_alp_sm_%.0f_dynamic_2.0/%.3f_%.3f_sit_%.1f\0",hdis,lon[i],lat[i],scoef[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],scoef[i]*alpsit[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_dynamic_2.0/%.3f_%.3f_prp\0",hdis,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpprp[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_dynamic_2.0/%.3f_%.3f_raw\0",hdis,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpraw[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_dynamic_2.0/%.3f_%.3f_prp_crd\0",hdis,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpraw[i][j]+alpprp[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_dynamic_2.0/%.3f_%.3f_crd\0",hdis,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpcrd[j]);
      fclose(fout);
