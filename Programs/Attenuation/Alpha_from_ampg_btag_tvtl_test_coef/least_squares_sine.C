...
#define NDAT 500
int least_fit_sine (double *thetao, double *dato, int ndato, double *A0, double *A1, double *phi, double *std)
{
   int i, j, ii, ndat=0;
   double pi=3.14159265359;
   double theta[ndato], dat[ndato];
...
//cout<<"thetao: "<<thetao[i]<<"  dato: "<<dato[i]<<endl;
   ...
   for(i=0;i<ndat;i++) {
      A[0][i] = cos(theta[i]*pi/180.);
      A[1][i] = sin(theta[i]*pi/180.);
      A[2][i] = 1;
   }
   for(i=0;i<dim;i++) 
      for(j=i;j<dim;j++) {
   ...
   for(i=0;i<dim;i++) {
      AC[i]=0;
      for(ii=0;ii<ndat;ii++) AC[i] += A[i][ii]*dat[ii];
   }
