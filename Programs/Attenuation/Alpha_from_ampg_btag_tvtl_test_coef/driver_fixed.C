...
int main (int argc, char *argv[])
{
   ...
   double alpraw[NSTA][500], alpcrd[NSTA][500], alpsit[NSTA][500], alpprp[NSTA][500], vel[NSTA][500], alperr[NSTA], velerr[NSTA], scoef[NSTA], azi[NSTA][500], aziv[NSTA][500];
   char sta[NSTA][10];
   double lonmin=360, lonmax=0, latmax=-90, latmin=90;
   int i, j, iev, ilon, ilat, ist, nst, nsta, nstt, nbad, nvel[NSTA], nvele[NSTA], nalp[NSTA], nalpe[NSTA];
   double per=60., pi=3.1415926536, alpmn=-0.003, alpmx=0.003, hdis=0.;;
...
//*/
   ...
   //double slpmn=, slpmx=;
   int count, icoef;
   sprintf(buff,"mkdir -p test_alp_sm_%.0f_fixed_1.3", hdis);
   system(buff);

   for(i=0;i<nst;i++) {
      if(nvel[i]<5) continue;
      sort2(&aziv[i][0], &vel[i][0], nvel[i]);
      sprintf(buff,"test_alp_sm_%.0f_fixed_1.3/%.3f_%.3f_vel\0",hdis,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nvel[i];j++) fprintf(fout,"%f %lf\n",aziv[i][j],vel[i][j]);
      fclose(fout);
      
      discard_by_std (&vel[i][0], nvel[i], std_coef, &count);
      least_fit_sine (&aziv[i][0], &vel[i][0], nvel[i], &A0, &A1, &phi, &std);
   ...
   for(i=0;i<nst;i++) { 
      ...
      sprintf(buff,"test_alp_sm_%.0f_fixed_1.3/%.3f_%.3f_sit_%.1f\0",hdis,lon[i],lat[i],scoef[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],scoef[i]*alpsit[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_fixed_1.3/%.3f_%.3f_prp\0",hdis,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpprp[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_fixed_1.3/%.3f_%.3f_raw\0",hdis,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpraw[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_fixed_1.3/%.3f_%.3f_prp_crd\0",hdis,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<nalp[i];j++) fprintf(fout,"%f %f %lf\n",azi[i][j],dist[i][j],alpraw[i][j]+alpprp[i][j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_fixed_1.3/%.3f_%.3f_crd\0",hdis,lon[i],lat[i]);
      fout=fopen(buff,"w");
...
//cout<<A0<<" + "<<A1<<" * sin( theta + "<<phi<<" )  std: "<<std<<endl;
      if(std>0.001) {nalpe[i]=-1; continue;}
