...
int Gauss_Smoothing(double *lon, double *lat, double *dat, double *densc, int nsta, double hdis)
{
   ...
   for(i=0;i<nsta;i++){
      ...
      //fclose(f1);
      if(ndata<3) {dsmd[i]=dat[i]; densc[i]=1;}
      else gau_fit_line(&dist[0], &data[0], ndata, slpmn, slpmx, alpha, &dsmd[i], &densc[i]);
      //dsmd[i]=itcpt;
   }

   for(i=0;i<nsta;i++) dat[i] = dsmd[i];
