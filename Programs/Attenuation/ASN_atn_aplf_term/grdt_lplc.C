...
//int calc_grdt_local(char *stalist, char*csta, char *fflag, char *dirlist, double *slong, double *slati, double *sdat, double *grdt, double *azi, int *nsta)
//{
//
//}
...
{
   FILE *ff;
   char buff[300], buff2[300], file[100];
   float longmin=360, longmax=0, latimin=90, latimax=-90, longtmp, latitmp;
   double radius=6371.1391285, pi=4.0*atan(1.0), dgr=0.1, dattmp;
   double grdtx, grdty, dis, alpha;
   double weight;
   int i, j, ii, jj, iper, ista, nrand, ntmpp, nstl;
...
//   }
  ...
  for(iper=0;iper<nper;iper++) {
   ...
   sprintf(file,"%s/Ph_Amp_Map_%.0fsec/%s_center_ph_amp_map_v2\0",dir,perlst[iper],csta);
   sprintf(buff,"/home/tianye/code/Script/GMT/C_plot_travel_positive %s region_%d %f", file, nrand, dgr);
   system(buff);

   for(ista=0;ista<*nsta;ista++) azi[iper][ista] = dat_flag[iper][ista];
   Check_azi_cov(&slong[0], &slati[0], *nsta, 200, &azi[iper][0]);
...
//            else gradtr[i][j]=(gradtr[i][j]+grdtmp)/2.;
   ...
   for(ista=0;ista<*nsta;ista++){
      ...
      ii=int((slong[ista]-longmin)/dgr);
      jj=int((slati[ista]-latimin)/dgr);
      ntmpp=0; weight=0; grdx[ista]=0, grdy[ista]=0, grdt[iper][ista]=0;
      for(i=ii;i<ii+2;i++)
         for(j=jj;j<jj+2;j++){
            if(i==0||j==0||i>npts_long-2||j>npts_lati-2) continue;
            if(gradtr[i][j]>=1e10) continue;
      ...
      if(ntmpp<2){ azi[iper][ista]=-1;; continue; }
      grdx[ista]=grdx[ista]/weight;
      grdy[ista]=grdy[ista]/weight;
      grdt[iper][ista]=sqrt(pow(grdx[ista],2)+pow(grdy[ista],2));
      azi[iper][ista]=pi/2-atan2(grdy[ista],grdx[ista]);
      if(azi[iper][ista]<0-1e-8)azi[iper][ista]+=2*pi;
      nstl++;
   }

   if(nstl<10) {
      cout<<perlst[iper]<<"sec: "<<nstl<<" stations left. Not enough for computing lplc map."<<endl;
      for(ista=0;ista<*nsta;ista++) azi[iper][ista]=-1;
      continue;
   }
   ...
   for(ista=0;ista<*nsta;ista++){ 
      ...
      ii=int((slong[ista]-longmin)/dgr);
      jj=int((slati[ista]-latimin)/dgr);
      ntmpp=0; weight=0; lplc[iper][ista]=0;
      for(i=ii;i<ii+2;i++)
         for(j=jj;j<jj+2;j++){
            if(i==0||j==0||i>npts_long-2||j>npts_lati-2) continue;
            if(gradtr[i][j]>=1e10) continue;
            ntmpp++;
            dis=sqrt(pow((longmin+i*dgr-slong[ista])*dislong[j],2)+pow((latimin+j*dgr-slati[ista])*dislati,2))+1e-20;
            lplc[iper][ista]+=gradtr[i][j]/dis;
            weight+=1/dis;
         }
      if(ntmpp<2) { azi[iper][ista]=-1; continue; }
      lplc[iper][ista]=lplc[iper][ista]/weight;
   }

   sprintf(buff,"rm -f %s_grdx %s_grdx.HD %s_grdy %s_grdy.HD", file, file, file, file);
   system(buff);
  }
