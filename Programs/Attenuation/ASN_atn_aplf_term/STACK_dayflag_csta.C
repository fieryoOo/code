...
int STACK_dayflag_csta ( char **staname, float *stalon, float *stalat, int nsta, char **dirlist, int ndir, char *outdir ){
  int i,j,k,npath,ii,jj,jjj,jjjj,flag;
  short day_fs[32];
  char outname[100],outfname[300],lst_name[300],outdir1[300],tstr[300],buff[100],dstr[32],pstr[NSTA*NSTA/2][100];
...
//    }
  ...
  sprintf(tstr,"mkdir -p %s\0" , outdir);
  system(tstr);

  fprintf (stderr,"%d stations, %d months\n",nsta,ndir);
  ...
  sprintf(tname1,"%s/Cor_dayflag.lst\0",outdir);
...
//  for (i=0;i<nsta;i++) {
     sprintf(outdir1,"%s/%s\0",outdir,staname[0]);
	...
	system(tstr);
     ...
     for (j=0;j<nsta;j++) {
       ...
       sprintf(outfname,"%s/%s/COR_%s_%s.SAC",outdir,staname[0],staname[0],staname[j]);
       flag = 0;
