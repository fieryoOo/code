...
int main (int argc, char *argv[])
{
   ...
   double weitsum[NSTA];
   double pi=3.1415926536, R=6367.5, hsmt=atof(argv[4]), pcoef=atof(argv[5]), scoef=atof(argv[6]);
   double hsco=1., gausco=0.5/1./1., std_coef=2.0, std_m=0.002, A1_m=0.0015, hcap=10., gaucap=0.5/hcap/hcap, azi_gap=250., scoef_mn=-0.2, scoef_mx=2.2;
/*-----------------------------input parameters--------------------------------------
...
*///---------------------------------------------------------------------------------
   ...
   if((fscanf(ff, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf", &hsmt, &hcap, &pcoef, &scoef, &hsco, &scoef_mn, &scoef_mx, &std_m, &A1_m, &azi_gap))!=10) {
      cout<<"Incorrect # of parameters in file "<<argv[4]<<endl;
      cout<<"Shoud have: h_smoothing h_cap prp_coef sit_coef h_scoef scoef_min scoef_max std_min A1_min azi_gap"<<endl;
      return 0;
   }
   fclose(ff);
 ...
 //for(i=0;i<nst;i++) cout<<lon[i]<<" "<<lat[i]<<"  beta: "<<beta[i]<<"  grdb: "<<grdb[i]<<"  azib: "<<azib[i]<<endl;
   ...
   sprintf(buff,"mkdir -p ev.dis_azi_vel_dec_sit_prp_alp.sm%.1f",hsmt);
   system(buff);
   double *densc = NULL;
   for(iev=0;;iev++){
      if(fgets(buff, 300, ff) == NULL ) break;
      ...
      tmpc = strdup(ampf);
      tmpc = strtok(tmpc, "_");
      sprintf(iname,"ev.dis_azi_vel_dec_sit_prp_alp.sm%.1f/%s.dis_azi_vel_dec_sit_prp_alp.txt", hsmt, tmpc);
      if((fin=fopen(iname,"r"))==NULL) {
         if((fout=fopen(ampf,"r"))==NULL) continue;
         fclose(fout);
         if((fout=fopen(tvtf,"r"))==NULL) continue;
         fclose(fout);
         calc_smth_grdt(ampf, along, alati, amp, hsmt, grda, azia, &nsta);
   ...
   //double slpmn=, slpmx=;
   int count, fgap;
   sprintf(buff,"mkdir -p test_alp_sm_%.0f_%.1f_%.1f", hsmt, pcoef, scoef);
   system(buff);
   ...
   for(i=0;i<nst;i++) {
      ...
      sprintf(buff,"test_alp_sm_%.0f_%.1f_%.1f/%.3f_%.3f_vel\0",hsmt,pcoef,scoef,lon[i],lat[i]);
      fout=fopen(buff,"w");
      ...
      fgap=1;
      if(vel[i][0]!=-1 && vel[i][nvel[i]-1]!=-1) if(vel[i][0]+360.-vel[i][nvel[i]-1]>azi_gap) fgap=-1;
      if(fgap==1) for(j=1;j<nvel[i];j++) {
         if(vel[i][j]==-1) continue;
         if(vel[i][j]-vel[i][j-1]>azi_gap) { fgap=-1; break; }
      }
   ...
   for(i=0;i<nst;i++) { 
    ...
    //cout<<ntmp<<": a "<<azitmp[ntmp]<<" d "<<distmp[ntmp]<<" r "<<rawtmp[ntmp]<<" p "<<prptmp[ntmp]<<" s "<<sittmp[ntmp]<<" w "<<weight[ntmp]<<" ws "<<weitsum[i]<<endl;
      ...
      fct=1e9;
      for(coef=scoef_mn;coef<=scoef_mx;coef+=0.2){
      ...
      sprintf(buff,"test_alp_sm_%.0f_%.1f_%.1f/%.3f_%.3f_sit\0",hsmt,pcoef,scoef,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<ntmp;j++) fprintf(fout,"%f %f %lf %lf %lf\n",azitmp[j],distmp[j],sittmp[j],deptmp[j],magtmp[j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_%.1f_%.1f/%.3f_%.3f_prp\0",hsmt,pcoef,scoef,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<ntmp;j++) fprintf(fout,"%f %f %lf %lf %lf\n",azitmp[j],distmp[j],prptmp[j],deptmp[j],magtmp[j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_%.1f_%.1f/%.3f_%.3f_raw\0",hsmt,pcoef,scoef,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<ntmp;j++) fprintf(fout,"%f %f %lf %lf %lf\n",azitmp[j],distmp[j],rawtmp[j],deptmp[j],magtmp[j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_%.1f_%.1f/%.3f_%.3f_prp_crd\0",hsmt,pcoef,scoef,lon[i],lat[i]);
      fout=fopen(buff,"w");
      for(j=0;j<ntmp;j++) fprintf(fout,"%f %f %lf %lf %lf\n",azitmp[j],distmp[j],rawtmp[j]+prptmp[j],deptmp[j],magtmp[j]);
      fclose(fout);
      sprintf(buff,"test_alp_sm_%.0f_%.1f_%.1f/%.3f_%.3f_crd\0",hsmt,pcoef,scoef,lon[i],lat[i]);
      fout=fopen(buff,"w");
...
//cout<<"slpr: "<<slpr<<"  stdr: "<<stdr<<"  avgr: "<<avgr<<"  scoef: "<<scoef[i]<<endl;
      ...
      fgap=1;
      if(crdtmp[0]!=-1 && crdtmp[ntmp-1]!=-1) if(azitmp[0]+360.-azitmp[ntmp-1]>azi_gap) fgap=-1;
      if(fgap==1) for(j=1;j<ntmp;j++) {
         if(crdtmp[j]==-1) continue;
         if(azitmp[j]-azitmp[j-1]>azi_gap) { fgap=-1; break; }
      }
      A1=0;
