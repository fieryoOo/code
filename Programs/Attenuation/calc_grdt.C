...
int calc_grdt(char *file, double *slong, double *slati, double *sdat, double *grdt, double *azi, int *nsta)
{
   ...
   system(buff);
   sprintf(buff,"/home/tianye/code/Script/GMT/C_plot_travel_positive %s.HD region_%d %f", file, nrand, dgr);
   system(buff);
...
{
   if(argc!=2){
      cout<<"usage:csta_map_gradient [input_map]"<<endl;
      return 0;
   }
   FILE *ff;
   char fname[100], *tmpc;
   double grdt[2000], azit[2000], adat[2000];
   double tlong[2000], tlati[2000], dist[2000];
   double clong, clati;
   int i, nstt;

   calc_grdt(argv[1], tlong, tlati, adat, grdt, azit, &nstt);
...
//   }
   ...
   sprintf(fname,"%s_grdt",argv[1]);
   ff=fopen(fname,"w");
   for(i=0;i<nstt;i++){
      if(azit[i]==-1) continue;
      fprintf(ff,"%8.4f  %8.4f  %8g\n", tlong[i], tlati[i], grdt[i]);
   }
   fclose(ff);
/*
   sprintf(fname,"%s_lplc",argv[1]);
   ff=fopen(fname,"w");
   for(i=0;i<nstt;i++){
      if(azit[i]==-1) continue;
