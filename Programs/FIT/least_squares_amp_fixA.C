...
int main (int argc, char *argv[])
{
  ...
  double J0,J0x[iJ0],J0y[iJ0],A0,alpha,alphamx,alphamn,alphastp,alphafit,alphag[100];
  float phvel,omega;

  if(argc != 5)
    {
      printf("Usage: least_squares_line.C [input file] [per] [phvel] [Amp]\n");
      exit(-1);
    }
...
//             cout<<J0x[jj]<<"  "<<J0<<endl;
             rsd[j]+=pow((J0*(2*pow(A0,2)/alpha)*exp(-alpha*datax[ii])-datay[ii]),2);
            }
          rsd[j]=sqrt(rsd[j]/(idata-1));
         }
    for(i=0;i<3;i++)
       for(j=i;j<10;j++)
          if(rsd[i]>rsd[j]){
