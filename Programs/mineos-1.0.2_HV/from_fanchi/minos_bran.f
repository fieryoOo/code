...
c MB added one line below
      ...
      open(3,file=filnam,form='formatted',iostat=iret)
...
c       print 200,nord,ichar(jcom),l,cvel,wmhz,tcom,gcom,qmod,wdiff
  ...
  200   format(i5,a2,i5,6g16.7)
        call modout(b,qmod,gcom,ioeig,ncall,tcom)
      else
        cg=5000./vn
...
c        print 200,nord,ichar(jcom),l,cvel,wmhz,tcom,gcom
      ...
      subroutine modout(wcom,qmod,gcom,ioeig,ncall,tcom)
      implicit real*8(a-h,o-z)
      parameter (mk=350)
...
c      write(ioeig,*) 'test',abuf(1),abuf(2),abuf(3),abuf(4),abuf(5)
c      do i=1,n
c         write(ioeig,300) i,abuf(5+i),abuf(5+n+i),
c     1    abuf(5+2*n+i),abuf(5+3*n+i),abuf(5+4*n+i),
c     2    abuf(5+5*n+i),tcom 
      write(ioeig,*) tcom,-1*abuf(5+2*n+n)/abuf(5+n)
c         write(ioeig,300) (abuf(i),i,nvec)
c      enddo
  300 format(i3,7f9.5)
      return
      end
