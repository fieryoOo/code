...
int TransferEvr(int ne, int ns, float **sig, SAC_HD *sd, int ithread) {
   ...
   *sig = NULL;
   if( (read_sac(sdb->rec[ne][ns].fname, sig, sd))==NULL ) {
      reports[ithread].tail += sprintf(reports[ithread].tail, "*** Warning: cannot read sac file %s ***", sdb->rec[ne][ns].fname);     
...
void * RmRESPEntrance( void * tid ) {
   ...
   for(;;) {
	 ...
	 if(sdb->rec[ne][ns].n > 0 ) if( !TransferEvr(ne, ns, &sig, &shd, ithread) ) continue; //sig allocated here
