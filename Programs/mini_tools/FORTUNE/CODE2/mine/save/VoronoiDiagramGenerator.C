...
struct Halfedge * VoronoiDiagramGenerator::ELleftbnd(struct Point *p)
{
	...
	if (he==ELleftend  || (he != ELrightend && p->x > Breakpoint(he,p->y)))
	{
		do 
		{
			he = he -> ELright;
		} while (he!=ELrightend && p->x > Breakpoint(he,p->y));	//keep going right on the list until either the end is reached, or you find the 1st edge which the point
		...
		{
			he = he -> ELleft;
		} while (he!=ELleftend && p->x <= Breakpoint(he,p->y));
...
struct Site * VoronoiDiagramGenerator::rightreg(struct Halfedge *he)
{
	...
	//if the ELpm field is zero, return the site 1 that this edge bisects, otherwise return site number 0
...
struct Edge * VoronoiDiagramGenerator::bisect(struct Site *s1,struct	Site *s2)
{
	...
	dy = s2->coord.y - s1->coord.y;
        newedge->a = dx/dy;
        newedge->b = 1.;
        //newedge->c = -(s2->coord.y + newedge->a * s2->coord.x);
//        newedge -> c = (float)(s1->coord.x * dx + s1->coord.y * dy + (dx*dx + dy*dy)*0.5)/dy; 
        float x1 = s1->coord.x, x2 = s2->coord.x, y1 = s1->coord.y, y2 = s2->coord.y;
        newedge -> c = 0.5 * (x2*x2 - x1*x1 + y2*y2 - y1*y1) / dy;
...
struct Site * VoronoiDiagramGenerator::NewVertex(struct Halfedge *el1, struct Halfedge *el2, struct Point *p)
{
	struct	Edge *e1,*e2, *e;
	struct  Halfedge *el;
	float d, xint, yint;
...
// computes the current breakpoint of halfedge el
float VoronoiDiagramGenerator::Breakpoint(struct Halfedge *el,float ysl) {
	struct Edge *e = el -> ELedge;
	struct Site *site = e -> reg[0];
	float x1 = site -> coord.x, y1 = site -> coord.y;
	site = e->reg[1];
	float x2 = site -> coord.x, y2 = site -> coord.y;

	float a = y2-y1;
	float b = 2*(x1*(ysl-y2)+x2*(y1-ysl));
	float c = -(ysl-y1)*(ysl-y2)*(y2-y1)+x1*x1*(y2-ysl)-x2*x2*(y1-ysl);
	float k = b*b-4*a*c;
        if( k<0 ) {
           printf("Error! need debug!\n");
           return -1;
        }
        if(el->ELpm == le) return (-b-sqrt(k))/2./a;
...
bool VoronoiDiagramGenerator::voronoi(int triangulate)
{
	...
	while(1)
	{
		...
		{/* new site is smallest - this is a site event*/
			...
			if ((p = NewVertex(lbnd, bisector)) != (struct Site *) NULL) 	//if the new bisector intersects with the left edge, remove the left edge's vertex, and put in the new one
			{	
				PQdelete(lbnd);
			...
			if ((p = NewVertex(bisector, rbnd)) != (struct Site *) NULL)	//if this new bisector intersects with the right edge,
			{	
		...
		{	
			...
			llbnd = lbnd -> ELleft;						//get the HalfEdge to the left of the above HE
			rbnd = lbnd -> ELright;						//get the HalfEdge to the right of the above HE
			rrbnd = rbnd -> ELright;					//get the HalfEdge to the right of the HE to the right of the lowest HE 
			...
			v -> sitenbr = nvertices;					//set the vertex number
        		nvertices += 1;
			...
			if((p = NewVertex(llbnd, bisector)) != (struct Site *) NULL)
			{	
				PQdelete(llbnd);
			...
			if ((p = NewVertex(bisector, rrbnd)) != (struct Site *) NULL)
			{	
