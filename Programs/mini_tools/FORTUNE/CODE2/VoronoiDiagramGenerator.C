...
struct Halfedge * VoronoiDiagramGenerator::ELleftbnd(struct Point *p)
{
	...
	/* Now search linear list of halfedges for the correct one */
        int flag1=-2, flag2=-2;
        if(he != ELleftend && he != ELrightend) {
           flag1 = right_of(he,p);
           flag2 = right_of_2(he,p,flag1);
           printf("%d %d\n", flag1, flag2);
        }
	if (he==ELleftend  || (he != ELrightend && flag1))
	{
		do 
		{
			he = he -> ELright;
...
int VoronoiDiagramGenerator::right_of_2(struct Halfedge *el,struct Point *p,int flag) {
	struct Edge *e = el -> ELedge;
	struct Site *site = e -> reg[0];
	double x1 = site -> coord.x, y1 = site -> coord.y;
	site = e->reg[1];
	double x2 = site -> coord.x, y2 = site -> coord.y;

        //int right_of_site = p -> x > x2;
        //if(right_of_site && el -> ELpm == le) return(1);
        //if(!right_of_site && el -> ELpm == re) return (0);
	...
	double k = b*b-4*a*c;
        if( k<0 ) {
           printf("Error! debug it!\n");
           return -1;
        }
        double ac;
        if(el->ELpm == le) ac = (-b-sqrt(k))/2./a;
        else ac = (-b+sqrt(k))/2./a;
        //if( ac < x1 || ac > x2 ) {printf("%lf  ", ac); ac = (-b-sqrt(k))/2./a; printf("%lf\n", ac); }
        
        if( ac < p->x ) {
           if(flag!=1) printf("%lf %lf %lf %lf %lf  ac: %lf px: %lf\n", x1, y1, x2, y2, y3, ac, p->x);
           return 1;
        }
        if(flag!=0) printf("%lf %lf %lf %lf %lf  ac: %lf px: %lf\n", x1, y1, x2, y2, y3, ac, p->x);
        return 0;
}
