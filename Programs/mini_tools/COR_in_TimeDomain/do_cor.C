...
{
   if(argc != 3) {
      printf("Usage: do_cor [in_sac_list (file1 file2)] [out_sac]\n");
      exit(-1);
   }
   ...
   int i, bc, bo, dnum, ncut = 0;
   int lag = 500, nhalf = 0;
   float cor[2*lag*50+1], ftmp;
   ...
   for(dnum=0;fgets(buff, 300, flst)!=NULL;) {
     ...
     for(i=0; i<nhalf; i++) {
        ftmp = 0.;
        for(bc=nhalf-i+ncut,bo=ncut; bc<sdc.npts-ncut; bo++,bc++) ftmp += sigc[bc] * sigo[bo];
        cor[i] += ftmp/(sdc.npts-nhalf+i-2*ncut);
     }
     for(i=nhalf; i<=2*nhalf; i++) {
        ftmp = 0.;
        for(bc=ncut,bo=i-nhalf+ncut; bo<sdo.npts-ncut; bo++,bc++) ftmp += sigc[bc] * sigo[bo];
        cor[i] += ftmp/(sdo.npts+nhalf-i-2*ncut);
     }
     dnum ++;
     /***********output**********/
     if(dnum==1) {
