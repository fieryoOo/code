...
bool calc_dist(double lati1, double long1, double lati2, double long2, double *dist)
{
  int i,flag=0;
  double latio1,longo1,dlt_long;//dlt_lati;
  double Rx;//R;
  double U1,U2;
  double Ds,alpha2;
  //double ctr_angl;
  ...
  double pi = 3.1415926535898; //4.0*atan(1.0);
  double pio180 = 0.0174532925199433;
  double Ra = 6378.137, Rb = 6356.7523142;
  double f = 0.00335281066474748; //1/298.257223563;
...
i++;
numda = numda1;
double sinnumda = sin(numda), cosnumda = cos(numda);
double ftmp1 = cosU2*sinnumda, ftmp2 = cosU1*sinU2-sinU1*cosU2*cosnumda;
...
double ftmp3 = Ra*ftmp1, ftmp4 = Rb*ftmp2;
...
{
  double dlt_long;//dlt_lati;
  double U1,U2;

  double pi = 3.14159265358979324; //4.0*atan(1.0);
  double pio180 = 0.017453292519943295;
  //Ra = 6378.137;
  //Rb = 6356.7523142;
  double f = 0.0033528106647474805; //1/298.257223563;
  long1=long1-(int)floor(long1)/360*360;
  if(long1<0) long1+=360;
  long2=long2-(int)floor(long2)/360*360;
  if(long2<0) long2+=360;
...
*alpha1=atan2(cosU2*sin(dlt_long), cos(U1)*sin(U2) - sin(U1)*cosU2*cos(dlt_long))*180/pi;
//*alpha2=atan2(cosU1*sin(dlt_long), -sin(U1)*cosU2 + cos(U1)*sin(U2)*cos(dlt_long))*180/pi;
