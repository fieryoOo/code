...
int main(int na, char *arg[])
{
  if(na!=7)
    {
      cout<<"usage:travel_time_to_velocity_map station_morgan.lst min max N_bin out_name scale"<<endl;
      return 0;
    }
  ...
  double min,max,d_bin,scale;
  int N_bin;
  min=atof(arg[2]);
  max=atof(arg[3]);
  N_bin=atoi(arg[4]);
  scale=atof(arg[6]);
  sprintf(name_iso,"%s_scale_%s.iso",arg[5],arg[6]);
  sprintf(name_ani,"%s_scale_%s.ani",arg[5],arg[6]);
  sprintf(name_ani_n,"%s_scale_%s_ani_n",arg[5],arg[6]);  
...
//  char event_name[300];
  ...
  marker_i=int((256.0-x0+0.001)/dx);
  ...
  for(;;)
      ...
      for(i=0;i<npts_x;i++)
	{
      ...
      for(;;)
	{
	  if(fscanf(fin2,"%lf %lf %lf",&lon,&lat,&temp)==EOF) break;
	 ...
	 if(temp<0.5&&temp>0.1&&laplace[i][j]<0.01&&laplace[i][j]>-0.01)
	   {
	     slow[i][j][n[i][j]]=sqrt(temp*temp-laplace[i][j]*scale);
