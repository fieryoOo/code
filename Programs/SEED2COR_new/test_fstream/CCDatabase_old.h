...
struct CCPARAM {
   ...
   int imonth;
   //-------------------------------------------------------------parameters----------------------------------------------------------//
   char rdsexe[300];			//rdseed excutable
   char evrexe[300];			//evalresp excutable
   char stafname[300];			//station list (J23A -129.6825 +44.844 optional_flag) (flag controls which sta-pairs to be ccd)
            		           	// (skip cc between sta-pairs that are 1> both flagged 0 or 2> both not 0 but flagged with different numbers)
   char seedfname[300];			//SEED file list (down_US_ALL/OBS_BHZ_2012.FEB.29.203762.seed 2012 2 29)
   ...
   char fwname[300];			//input spectrum reshaping signal file (takes effect only when frechlen==-1)
