...
This code extracts information (distance, azimuth, velocity, amplitude, and snr) from FTAN results of earthquakes.
   The lag(s) with SNR lower than snrmin are considered bad measurement(s) and are rejected.
...
{
   if(na!=5) {
      cout<<"usage: "<<arg[0]<<" [station.lst] [sta1-sta2-Dispf-SNR list] [period] [min snr]"<<endl;
      return 0;
   }
...
//read in sta1, sta2, dispf, snrf from file list
   ...
   for(i=0;fgets(buff, 500, ff)!=NULL;i++) {
      ...
      sscanf(buff,"%s %s %s %s", spr[i].sta1, spr[i].sta2, spr[i].disp_pf, spr[i].snr_pf);
   }
   npth = i;
   fclose(ff);
