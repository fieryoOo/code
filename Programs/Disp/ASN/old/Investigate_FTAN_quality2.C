...
int AmpSNRSummation( char *fdisp, char *fsnr, char *fgrvpred, char *fphvpred, float *snr, float *amp, struct LAG_DATA *data ) {
  ...
  // omp_set_lock(&readlock);
   ...
   if( ghlgrv < 1. ) {
      ...
      nfgrv = i;
      // define period range
      perlow = pergrv[0]; perhigh = pergrv[0];
      for(i=1; i<nfgrv; i++) {
   ...
   *snr = 0.; *amp = 0.;
   ...
   while(fgets(buff1, 300, fin1) && fgets(buff2, 300, fin2)) {
      ...
      // compute distance to predictions. set to one gaussian halflength if predictions are to be ignored (so that the gaussian weight will be about 0.6)
      disgrv = ghlgrv; disphv = ghlphv;
      if( ghlgrv < 1. ) disgrv = FVDist(percur, grvcur, pergrv, grvpred, nfgrv);
      if( ghlphv < 1. ) disphv = FVDist(percur, phvcur, perphv, phvpred, nfphv);
      // save data
      ...
      for(i=0;i<wsnr_pow;i++) weight *= snrcur;
      *snr += weight*snrcur;
      *amp += weight*ampcur;
      weit += weight;
      neff++;
