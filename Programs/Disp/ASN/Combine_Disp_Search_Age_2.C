...
/*
   if(per==6) { ccl=0., cch=2., csl=-1., csh=1., cll=-0.5, clh=0.5; }
   else if(per==6.5) { ccl=0., cch=2., csl=-1., csh=1., cll=-0.5, clh=0.5; }
   else if(per==7.0) { ccl=0.5, cch=2.5, csl=-1.5, csh=0.5, cll=-0.5, clh=0.5; }
   else if(per==7.5) { ccl=1.5, cch=3.5, csl=-2., csh=-0., cll=-0.5, clh=0.5; }
   else if(per==8.0) { ccl=2.5, cch=4.5, csl=-3., csh=-1., cll=0., clh=1.; }
   ...
   else if(per==10.0) { ccl=2.5, cch=4.5, csl=-1., csh=1., cll=-0.5, clh=0.5; }
   else if(per==11.0) { ccl=2.5, cch=4.5, csl=-1., csh=1., cll=-0.5, clh=0.5; }
   else if(per==12.0) { ccl=2.5, cch=4.5, csl=-0.5, csh=1.5, cll=-0.5, clh=0.5; }
   else if(per==13.0) { ccl=2.5, cch=4.5, csl=-0.5, csh=1.5, cll=-0.5, clh=0.5; }
   else if(per==14.0) { ccl=2., cch=4., csl=0., csh=2., cll=-0.75, clh=0.25; }
   else if(per==15.0) { ccl=2., cch=4., csl=0., csh=2., cll=-1., clh=0.; }
   else if(per==16.0) { ccl=2.5, cch=4.5, csl=-1., csh=1., cll=-0.5, clh=0.5; }//
   else if(per==17.0) { ccl=2., cch=4., csl=-0.5, csh=1.5, cll=-0.5, clh=0.5; }
   else if(per==18.0) { ccl=2., cch=4., csl=-0.5, csh=1.5, cll=-0.5, clh=0.5; }
...
//extract dist and age info for each path
   ...
   int ipt, ne, icc, ics, icl, ifix;
   int nstepc, nsteps, nstepl;
   float dcc, dcs, dcl;
   float cct, cst, clt, rsdt, rsdmin = 100., rsdm, rstd, T, vel;
   float rsd1[iper], rsd2[iper];
   char name[100];
   FILE *ff;
   for(ifix=0;ifix<3;ifix++) {
      cout<<"Fixed cc"<<ifix<<"..."<<endl;
      ccl = 0.; cch = 4.5; nstepc = 225; dcc = (cch-ccl)/(nstepc-1);
      csl = -3.; csh = 3.; nsteps = 300; dcs = (csh-csl)/(nsteps-1);
      cll = -1.; clh = 0.3; nstepl = 130; dcl = (clh-cll)/(nstepl-1);
      if(ifix==0) { ccl = 0.; nstepc = 1; }
      else if(ifix==1) { csl = 0.; nsteps = 1; }
      else { cll = 0.; nstepl = 1; }
      sprintf(name,"rsdg_fixc%d_%.1f.txt",ifix,per);
      ff = fopen(name,"w");
      for(cct=ccl,icc=0;icc<nstepc;icc++,cct+=dcc)
         for(cst=csl,ics=0;ics<nsteps;ics++,cst+=dcs)
            for(clt=cll,icl=0;icl<nstepl;icl++,clt+=dcl) {
             //calc rsd
               for(i=0;i<iper;i++) {
                  if(npt[i]==0) { rsd1[i] = -1.; continue; }
                  T=0.;
                  ...
                  vel = dist[i]/T;
                  rsd1[i] = fabs(vel-grv[i]);
                  rsd2[i] = pow(rsd1[i],2);
               }
             //discard outliers
               for(ne=0,rsdt=0,i=0;i<iper;i++) if(rsd1[i]>0) { rsdt+=rsd2[i]; ne++; }
               rsdt /= ne;
               for(rstd=0,i=0;i<iper;i++) if(rsd1[i]>0) rstd+=pow((rsd2[i]-rsdt),2);
               rstd = sqrt(rstd/(ne-1));
//cout<<"   "<<ne<<" - ";
               if(rsd1[0]>0 && rsd2[0]<rsdt+rstd*2.5) ne=1;
               else { rsd1[0]=0.; rsd2[0]=0.; ne=0; }
               for(i=1;i<iper;i++) if(rsd1[i]>0 && rsd2[i]<rsdt+rstd*2.5) { rsd1[0]+=rsd1[i]; rsd2[0]+=rsd2[i]; ne++; } //else {cout<<rsd[i]<<" ";} cout<<endl;
               rsd1[0] = rsd1[0]/(ne-1);
               rsd2[0] = sqrt(rsd2[0]/(ne-1));
//cout<<ne<<endl;
               fprintf(ff, "%f %f %f %f %f %d\n", cct, cst, clt, rsd1[0], rsd2[0], ne );
               if(rsdmin>rsd2[0]) {
                  rsdmin = rsd2[0];
                  *cc = cct;
                  *cs = cst;
                  *cl = clt;
                  for(i=0;i<iper;i++)
                     if(rsd1[i]>0 && rsd2[i]<rsdt+rstd*2.5) flag[i] = 1;
                     else flag[i] = 0;
...
void Search_ph_age(float per, float *phv, float *lon1, float *lat1, float *lon2, float *lat2, int iper, float *cc, float *cs,  float *cl, int *flag, char *fname) {
   ...
   float ccl, cch, csl, csh, cll, clh;
/*
   if(per==6) { ccl=2.2, cch=2.7, csl=-0.3, csh=0., cll=-0.2, clh=0.1; }
   else if(per==6.5) { ccl=2.5, cch=3., csl=-0.5, csh=-0.2, cll=-0.2, clh=0.1; }
   else if(per==7.0) { ccl=2.8, cch=3.3, csl=-0.35, csh=-0.05, cll=-0.2, clh=0.1; }
   else if(per==7.5) { ccl=3.1, cch=3.6, csl=-0.35, csh=-0.05, cll=-0.2, clh=0.1; }
   else if(per==8.0) { ccl=3.1, cch=3.6, csl=-0.1, csh=0.2, cll=-0.3, clh=0.; }
   ...
   else if(per==10.0) { ccl=3.1, cch=3.6, csl=0.1, csh=0.4, cll=-0.3, clh=0.; }
   else if(per==11.0) { ccl=3.1, cch=3.6, csl=0.3, csh=0.6, cll=-0.3, clh=0.; }
   else if(per==12.0) { ccl=3.1, cch=3.6, csl=0.3, csh=0.6, cll=-0.3, clh=0.; }
   else if(per==13.0) { ccl=3.1, cch=3.6, csl=0.3, csh=0.6, cll=-0.3, clh=0.; }
   else if(per==14.0) { ccl=3.1, cch=3.6, csl=0.2, csh=0.5, cll=-0.2, clh=0.1; }
   else if(per==15.0) { ccl=3.2, cch=3.7, csl=0.2, csh=0.5, cll=-0.2, clh=0.1; }
   else if(per==16.0) { ccl=3.3, cch=3.8, csl=0., csh=0.3, cll=-0.15, clh=0.15; }
   else if(per==17.0) { ccl=3.3, cch=3.8, csl=-0.05, csh=0.25, cll=-0.1, clh=0.2; }
   else if(per==18.0) { ccl=3.3, cch=3.8, csl=0., csh=0.3, cll=-0.2, clh=0.1; }
...
//extract dist and age info for each path
   ...
   for(ifix=0;ifix<3;ifix++) {
      cout<<"Fixed cc"<<ifix<<"..."<<endl;
      ccl = 2.; cch = 4.; nstepc = 400; dcc = (cch-ccl)/(nstepc-1); //4 times
      csl = -0.8; csh = 1.3; nsteps = 700; dcs = (csh-csl)/(nsteps-1); //6 times
      cll = -0.4; clh = 0.5; nstepl = 300; dcl = (clh-cll)/(nstepl-1); //2 times
      if(ifix==0) { ccl = 0.; nstepc = 1; }
      else if(ifix==1) { csl = 0.; nsteps = 1; }
      else { cll = 0.; nstepl = 1; }
      sprintf(name,"rsdp_fixc%d_%.1f.txt",ifix,per);
      ff = fopen(name,"w");
      for(cct=ccl,icc=0;icc<nstepc;icc++,cct+=dcc)
         for(cst=csl,ics=0;ics<nsteps;ics++,cst+=dcs)
            for(clt=cll,icl=0;icl<nstepl;icl++,clt+=dcl) {
