/*
This code find the amp and snr of FTAN results around the highest snr in a given period range
  Bad data are discarded according to the distance between measured and predicted dispersion curves.
  Output: (distance, azimuth, peak_amp, and peak_snr of positive and negative lags and
   ...
   *snr = 0.; *amp = 0.;
   ...
   while(fgets(buff1, 300, fin1) && fgets(buff2, 300, fin2)) {
	 ...
	 continue;
      ...
      ftmp = snrcur * snrcur * snrcur;
      weight *= (ftmp * ftmp * ftmp); // put most weights on larger snr
      *snr += weight*snrcur;
      *amp += weight*ampcur;
      weit += weight;
      neff++;
...
/* main loop. Process data from each path. Compute dist and snr, and as an indicator of FTAN quality,
   ...
   sprintf(buff, "Disp_peak_%.1f_%.1f", perl, perh);
   ff = fopen(buff, "w");
