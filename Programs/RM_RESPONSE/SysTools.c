...
void EstimateMemAvail (long *MemAvail) {
   ...
   //else
   cerr<<"### Warning: Failed to get cached memory size from /proc/meminfo!! ###"<<endl;
   cerr<<"### The current freeRAM ("<<*MemAvail/1024./1024.<<" Mb) will be used ###"<<endl;
   TimedContinue(10); 
}
...
char * wMove (char *pattern, char *tdir, int retlst, int *nfile) {
  // return a list of moved files when retlst==1
   ...
   char *list = List(".", pattern, 0, nfile);
