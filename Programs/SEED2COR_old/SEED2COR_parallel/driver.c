...
int GetParameters(char *fname) {
   ...
   if((fparam=fopen(fname,"r"))==NULL) {
      cerr<<"Error: Cannot open parameter file "<<fname<<endl;
      exit(0);
   }
   cerr<<"---------------------------Checking input parameters---------------------------"<<endl;
   //rdsexe
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%s", rdsexe);
   cerr<<"rdseed excutable:\t"<<rdsexe<<endl;
   if( ! strstr(rdsexe, "rdseed") ) cerr<<"   Warning: Are you sure this is an rdseed excutable?"<<endl;
   ...
   cerr<<"evalresp excutable:\t"<<evrexe<<endl;
   if( ! strstr(evrexe, "evalresp") ) cerr<<"   Warning: Are you sure this is an evalresp excutable?"<<endl;
   ...
   if((filetmp=fopen(seedlst,"r"))==NULL) {
      ...
      if(sscanf(buff,"%s %d %d %d", ctmp, &itmp, &itmp, &itmp)!=4) {
         cerr<<"   Error: incorrect format in file "<<seedlst<<"! (path/seedfile yyyy mm dd) is expected"<<endl;
         ERR = 1;
      }
      fclose(filetmp);
   }
   ...
   sscanf(buff, "%f", &ftmp);
   sps = (int)ftmp;
   cerr<<"target sampling rate:\t"<<ftmp<<endl;
   if( sps!=ftmp || sps <= 0 ) {
   ...
   cerr<<"max gap fraction:\t"<<gapfrac*100<<"%"<<endl;
   ...
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%f", &t1);
   cerr<<"cutting begining:\t"<<t1<<"sec"<<endl;
   if( fabs(t1) > 86400 ) cerr<<"   Warning: "<<t1<<"sec exceeds one day."<<endl;
   //tlen
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%f", &tlen);
   cerr<<"time-rec length:\t"<<tlen<<"sec"<<endl;
   ftmp = t1+tlen;
   if( ftmp>86400 || ftmp<0 ) cerr<<"   Warning: ending time '"<<ftmp<<"sec' out of range."<<endl;
   //perl perh
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%f", &perl);
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%f", &perh);
   cerr<<"signal per band:\t"<<perl<<" - "<<perh<<"sec"<<endl;
   if( perl<0 || perh<0 ) {
   ...
   if(perl<5./sps) cerr<<"   Warning: signal at "<<perl<<"sec might be lost at a sampling rate of "<<sps<<endl;
   //tnorm_flag
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%d", &tnorm_flag);
   cerr<<"t-norm method:\t\t";
   if(tnorm_flag==0) cerr<<"none"<<endl;
   else if(tnorm_flag==1) cerr<<"One-bit"<<endl;
   else if(tnorm_flag==2) cerr<<"Running average"<<endl;
   else if(tnorm_flag==3) cerr<<"Earthquake cutting"<<endl;
   else {
   ...
   sscanf(buff, "%f", &Eperh);
   if(tnorm_flag!=1) {
      if( Eperl == -1. ) cerr<<"Eqk filter:\t\toff"<<endl;
      else {
         cerr<<"Eqk filter:\t\t"<<Eperl<<" - "<<Eperh<<"sec"<<endl;
         if( Eperl<0 || Eperh<0 ) {
            cerr<<"   Error: period band can not go below 0."<<endl;
            ERR = 1;
         }
   ...
   if(tnorm_flag!=1) {
      cerr<<"t-len for run-avg:\t"<<timehlen<<endl;
      if(timehlen<0) {
   ...
   cerr<<"t-len for whitening:\t"<<frechlen<<"  ";
   if(frechlen==-1) cerr<<"input smoothing file will be used";
   else if(frechlen<0) {
   ...
   if(frechlen==-1) {
      cerr<<"spec reshaping file:\t"<<fwname<<endl;
   ...
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%d", &ftlen);
   if(ftlen<=0) cerr<<"cor-time-len correction\toff"<<endl;
   else {
      cerr<<"cor-time-len correction\ton"<<endl;
      if(tnorm_flag==3) ftlen = 2;
   }
   //fprcs
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%d", &fprcs);
   if(fprcs<=0) cerr<<"prcsr-signal checking\toff"<<endl;
   else cerr<<"prcsr-signal checking\ton"<<endl;
   //memomax
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%f", &memomax);
   cerr<<"memory fraction:\t"<<memomax*100<<"%"<<endl;
   ...
   cerr<<"cor lag time:\t\t"<<lagtime<<"sec"<<endl;
   if(lagtime<0) {
      cerr<<"   Error: negative lagtime!"<<endl;
      ERR = 1;
   }
   else if(lagtime>524288) cerr<<"   Warning: lag time exceeds the maximum"<<endl;
   //mintlen
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%d", &mintlen);
   cerr<<"min time len:\t\t"<<mintlen<<"sec"<<endl;
   if( mintlen>86400 ) cerr<<"   Warning: allowed minimum time length larger than a day."<<endl;
   //fdel1
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%d", &fdel1);
   cerr<<"Delete orig sacs?\t";
   if(fdel1==1) cerr<<"Yes"<<endl;
   else cerr<<"No"<<endl;
   //fdel2
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%d", &fdel2);
   cerr<<"Delete am&ph files?\t";
   if(fdel2==1) cerr<<"Yes"<<endl;
   else cerr<<"No"<<endl;
   //fskip1
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%d", &fskip1);
   cerr<<"ExtractSac()\t\t";
   if(fskip1==2) cerr<<"skip"<<endl;
   else if(fskip1==1) cerr<<"skip if target file exists"<<endl;
   else cerr<<"overwrite if target file exists"<<endl;
   //fskip2
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%d", &fskip2);
   cerr<<"RmRESP()\t\t";
   if(fskip2==2) cerr<<"skip"<<endl;
   else if(fskip2==1) cerr<<"skip if target file exists"<<endl;
   else cerr<<"overwrite if target file exists"<<endl;
   //fskip3
   if( (fgets(buff, 300, fparam)) == NULL ) return 0; 
   sscanf(buff, "%d", &fskip3);
   cerr<<"TempSpecNorm()\t\t";
   if(fskip3==2) cerr<<"skip"<<endl;
   else if(fskip3==1) cerr<<"skip if target file exists"<<endl;
   else cerr<<"overwrite if target file exists"<<endl;
   //fskip4
   if( (fgets(buff, 300, fparam)) == NULL ) return 0;
   sscanf(buff, "%d", &fskip4);
   cerr<<"CrossCorr()\t\t";
   if(fskip4==2) cerr<<"skip"<<endl;
   else if(fskip4==1) cerr<<"skip if target file exists"<<endl;
   else cerr<<"overwrite if target file exists"<<endl;
   //check for EOF
   if( (fgets(buff, 300, fparam)) != NULL ) cerr<<"   Warning: End of file not reached!"<<endl;
...
int main (int argc, char *argv[])
{
   ...
   if( ! GetParameters(argv[1]) ) {
      cerr<<"   Error: Parameter loading failed. Incorrect format from file "<<argv[1]<<endl;
      exit(-1);
   }
   FillStations();
   FillMonths();
   ...
   for(imonth=0;imonth<sdb->nmo;imonth++) {
	 ...
	 cerr<<"Skipping month "<<sdb->mo[imonth].name<<". (Month directory deleted)"<<endl;
	 continue;
      }
      cerr<<"Working on month "<<sdb->mo[imonth].name<<endl;
      FillEvents();
      ExtractSac();
      RmRESP();
      TempSpecNorm();
