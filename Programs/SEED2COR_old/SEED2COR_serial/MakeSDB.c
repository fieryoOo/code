...
void SortSeed(int *year, int *month, int *day, char **name, int n) {
   ...
   for(i=0;i<n;i++) {
      ...
      for(j=i;j>0&&(ytmp<year[j-1]||(ytmp==year[j-1]&&mtmp<month[j-1])||(ytmp==year[j-1]&&mtmp==month[j-1]&&dtmp<day[j-1]));j--) {
         day[j] = day[j-1];
         month[j] = month[j-1];
         year[j] = year[j-1];
...
void FillMonths() {
   ...
   if((fseed=fopen(seedlst,"r")) == NULL) {
      cout<<"Cannot open file "<<seedlst<<endl;
      exit(-1);
   }
   ...
   for(i=0;i<NMONTH*31;i++) {
      ...
      //cout<<i<<" "<<seedn[i]<<" "<<year[i]<<" "<<month[i]<<" "<<day[i]<<endl;
   }
   fclose(fseed);
   if(i==NMONTH*31) {
      cout<<"increase NMONTH!"<<endl;
      exit(-1);
   }
   nseed = i;
   ...
   for(i=0;i<nseed;i++) {
      if(i==0 || month[i]!=month[i-1]) {
         ...
         sprintf(buff, "mkdir -p %s", sdb->mo[sdb->nmo].name);
         system(buff);
...
void FillStations() {
   ...
   if((fsta=fopen(stalst,"r")) == NULL) {
      cout<<"Cannot open file "<<stalst<<endl;
      exit(0);
   }
   for (ist=0;;ist++) {
...
void FillEvents() {
   ...
   for(i=0,iev=0;i<NEVENTS;i++) {
      ...
      sdb->ev[i].t0 = abs_time (sdb->ev[i].yy, sdb->ev[i].jday, sdb->ev[i].h, sdb->ev[i].m, sdb->ev[i].s, sdb->ev[i].ms );
      sprintf(sdb->ev[i].name, "%s.%d", sdb->mo[imonth].name, sdb->ev[i].dd );
      sprintf(buff,"mkdir -p %s/%s", sdb->mo[imonth].name, sdb->ev[i].name );
      system(buff);
      iev++;
   }
   sdb->nev = iev;
