...
/* The program takes the sac files from sa_from_seed_RESP, removes the instrument response and cuts the file based on the user input parameters [t1] and [npts]. The program is hard-wired for a broadband signal, in the period band 5-150 s, but this can be changed in the one_rec_trans function. */
...
/*--------------------------------------------------------------------------*/
{
  ...
  if ( !read_sac ("s1.sac", sig1, &shd1, 5000000 ) ) {
    fprintf(stderr,"file %s not found\n", sd->rec[ne][ns].fname );
    return;
  }
