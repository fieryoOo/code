...
void DeleteGroup(int size) {
   int i, iev; 
   char fname[150];
   cerr<<"### Deleting am&ph files for the current station group... ###"<<endl;
   for(i=0; i<size; i++) { 
      for(iev=0; iev<NEVENTS; iev++) {
...
int GroupSize(int *spnpts) {
   ...
   *spnpts = npts;
   ...
   if( size < 1 ) {
      cerr<<"No enough memory for Crosscorrelating!! Increase max memmory% or switch to CrossCor_old.c "<<endl;
      exit(0);
   }
...
void PrepareCor(char *dirname, short ***dnum, short ****dflag) {
   ...
   *dflag = (short ***) malloc (sdbnew->nst * sizeof(short **));
   ...
   if( access(dirname, 0) != 0 ) { 
      ...
      MKDir(dirname);
      if(fskip4)
         cerr<<"   Cannot access "<<dirname<<". No Correlation will be skipped!"<<endl;
   }
   ...
   if((fd=fopen(str,"r"))==NULL) {
      if(fskip4)
         cerr<<"   Cannot access "<<str<<". No Correlation will be skipped!"<<endl;
      return;
   }
   char str2[300];
   sprintf(str, "%s/Cor_dayflag.lst", dirname);
...
void CrossCorr() {
   ...
   *sdbnew = *sdb;
   ...
   for ( ig1 = 0; ig1 < ng; ig1++ ) {
	 ...
	 ReadGroup(-1, is2, starec2);
	    ...
	    for( ithread=0;ithread<NTHRDS;ithread++ ) {
	       ...
	       rc = pthread_create( &tid[ithread], &attr_j, CorEvent, (void *) (&(targs[ithread])) );
	       if(rc) {
	          cerr<<"Creation failed! ERR: "<<strerror(rc)<<endl;
	          exit(0);
	       }
	    }
      ...
      fprintf(stderr, "%d station pairs (%d records) processed. ###\n", npair, nrec);
   }
   ...
   for( iev=0; iev<NEVENTS; iev++ ) { free(starec2[0].dayrec[iev].amp); free(starec2[0].dayrec[iev].pha); }
   delete [] starec2; starec2 = NULL;
