...
void FillMonths() {
   ...
   if((fseed=fopen(seedlst,"r")) == NULL) {
      cerr<<"Cannot open file "<<seedlst<<endl;
      exit(-1);
   }
   ...
   sdb->nmo = sdb->nmo + 1;
   cerr<<sdb->nmo<<" months filled"<<endl;
}
...
void FillStations() {
   ...
   if((fsta=fopen(stalst,"r")) == NULL) {
      cerr<<"Cannot open file "<<stalst<<endl;
      exit(0);
   }
   for (ist=0;;ist++) {
