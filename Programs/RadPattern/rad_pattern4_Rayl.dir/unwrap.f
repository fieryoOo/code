...
Ccall                  unwrap(dper,t,nt,temp_ph,unph,grt,r)
...
C           PRint*,dp,ph(i+1),ph(i),abs(ph(i+1)-ph(i)),i
                                ENDIF
           enddo
...
C        do j=1,kj
C       if(jump(j).eq.i) then
C       grt(i-1)=(ph(i-2)-ph(i-1))/2./dper/pi2*per(i-1)
C       grt(i)=(ph(i+1)-ph(i))/2./dper*q
C                        endif
C        enddo
               enddo
              m=0
