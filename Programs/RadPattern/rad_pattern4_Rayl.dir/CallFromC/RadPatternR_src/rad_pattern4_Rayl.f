      subroutine rad_pattern_r(eigfile,eiglen, phvfile,phvlen, 
...
c To calculate group_delay as a function of azimuth and period
      integer*4 ntmax
      parameter (ntmax=200)
      integer*4 eiglen, phvlen
      real*4 v(3,ntmax),dvdz(3,ntmax),ampr(ntmax)
      real*4 tm(6),du(3),vu(3),wvn(2),period(20),ampl_max(ntmax)
      complex*8 br(6),bl(6),sumr
      character*80, eigfile, phvfile
      character*40 bred
      character*2 symbik
...
C-----------read m and dper from .phv file ------
      open(1,file=phvfile(1:phvlen),status='OLD')
...
c-----------reading OLD_SURF_DEEP output------------c
      ...
      call surfread(eigfile(1:eiglen),sigR,sigL,symbik,nt,nd,depth
     +              ,t,cr,ur,wvr,cl,ul,wvl,v,dvdz,ampr)
