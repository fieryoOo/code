         subroutine init(dx,nlay_deriv,idispr,idispl,kind,ncount)
...
C       data ndpth/1000/,dpth0/0.0/,ddepth/1.0/,r0/6371.0/
        data ndpth/1000/,dpth0/0.0/,ddepth/0.5/,r0/6371.0/
C	data ndpth/180/,dpth0/0.0/,ddepth/0.01/,r0/6371.0/
...
C--------------INITIATION------------------------------------------S
	...
	KEY_CINIT=     .FALSE.
        ...
        idispl=0
c----------------------------------------------------------------------
c       idispl>0 - Love wave calculations 
c       idispr>0 - Rayleigh wave calculations 
c----------------------------------------------------------------------
        print*,'surface wave forward calculations for earthquake seismology problems:'
	print*,'options for flattenning, attenuation, partial derivatives,'
	print*,'eigenfunctions and their derivatives by depth'
C-------interpreting arguments--------------------------------S
        narg=iargc()
	if (narg.lt.8.or.narg.gt.17)then
	print*,'USAGE: SURF_DISP infile  outfile R/L kmin kmax tmin tmax tstep'
	print*,'[-f] [-a] [-s depth_step] [-d par_type nl dx] [-e[1/2/n]] [-c  cinit]'
	STOP
	endif
	  ...
	  call GETARG(3,symb)
        ...
        if(symb.eq.'L') kind=1
        if(symb.eq.'R') kind=2
        if(symb.eq.'L') idispl=1
        if(symb.eq.'R') idispr=1
...
C--------for initial phase velocity---------------------S
	 ...
	 read(symbol,*) cinit
	 print*,KEY_CINIT,'starting phase velocity=',cinit
		       endif
...
C--------for partial derivatives found by numerical technique-S
	  ...
	  lqq=ln+lt+ls+12 
	  der_phvfile1(1:lqq)=outfile(1:ln)//'_'//type(1:lt)//'_'//symbol(1:ls)//'.'//symb//'.phv_1st'
	  der_phvfile2(1:lqq)=outfile(1:ln)//'_'//type(1:lt)//'_'//symbol(1:ls)//'.'//symb//'.phv_2nd'
	  der_grvfile1(1:lqq)=outfile(1:ln)//'_'//type(1:lt)//'_'//symbol(1:ls)//'.'//symb//'.grv_1st'
	  der_grvfile2(1:lqq)=outfile(1:ln)//'_'//type(1:lt)//'_'//symbol(1:ls)//'.'//symb//'.grv_2nd'
...
c-------to define an array of periods--------------------------------S
	...
	kmax=nper
        ...
        PRINT *, 'NLAYERS=', mmax,' NUMB. OF PERIODS=',kmax
        t(1)=t1
                                do i=2,kmax
	t(i)=t(i-1)+dt
				enddo
c-------to define an array of periods--------------------------------E
        PRINT*,'    YOUR CHOICE:'
        PRINT*,'DDEPTH=',ddepth
	IF(symb.EQ.'R')PRINT*,'WAVE TYPE = RAYLEIGH   '
	IF(symb.EQ.'L')PRINT*,'WAVE TYPE = LOVE       '
	IF(KEY_FLAT)PRINT*,'FLATTENNING          '
	IF(KEY_ATTEN)PRINT*,'ANELASTICITY         '
	IF(KEY_EIGEN)PRINT*,'EIGEN FUNCTIONS      '
	IF(KEY_DERIV) then
	PRINT*,'PARTIAL DERIVATIVES for the ',nlay_deriv,'th LAYER'
	IF(KEY_VP)PRINT*,'by VP with perturbation=',dx,'km/s'
	IF(KEY_VS)PRINT*,'by VS with perturbation=',dx,'km/s'
	IF(KEY_RHO)PRINT*,'by RHO with perturbation=',dx,'g/cm**3'
	IF(KEY_TH)PRINT*,'by THICKNESS with perturbation=',dx,'km'
		     endif
        return
	end
