...
//printf("sta: %s  lst_sta: %s\n",sta[ii],stalst[i]);
  ...
  avg=0;
  weight=0;
  for (i=0;i<8;i++){bin_avg[i]=0;bin_weight[i]=0;sta_num[i]=0;}
  for (ii=1;ii<ista;ii++){
     ibin=(int)(deg[ii]/45.0);
     if(deg[ii]==360)ibin-=1;
...
//printf("bin_avg: %f  sta_num: %d\n",bin_avg[i],sta_num[i]);
  ...
  if(flag<7) printf("Station %s: Got coverage in only %d out of 8 directions, skipped!\n",sta[0],flag);
  else {
     printf("Station %s: Coverage in %d out of 8 directions.\nBin avg is: %f\n",sta[0],flag,avg/weight);
     longout=stalong[i];
     if(longout<0)longout+=360;
     fprintf(fout,"%f %f %f\n", longout, stalati[i], avg/weight);
    }
if(strcmp(filelist[ifile],"O10A_7.378866_daynormd_amp")==0)return 0;
 }
 fclose(fout);
 return 0;
}
