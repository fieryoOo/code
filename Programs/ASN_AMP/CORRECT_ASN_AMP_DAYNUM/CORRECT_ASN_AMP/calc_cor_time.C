...
int main(int argc, char *argv[])
{
  ...
  if(argc != 4)
    {
      printf("Usage: Calc_cor_time [rec_file_1] [rec_file_2] [output(1 for table; 2 for day_sum]\n");
      exit(-1);
    }
...
//           printf("%d\n",day_n);
     ...
     day_n=0;
     for(j=1;j<=366;j++){
            if(sig1[j]*sig2[j]>0)day_n++;
        }
     printf("%d\n",day_n);
    }
  else printf("Input 1 for table, 2 for day_num in total!\n");
//  day_n=tysac.npts;
}
