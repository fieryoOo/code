...
SAC_HD tysac;
/*--------------------------------------------------------------*/
int get_snr(char *fname,int day_num)
/*--------------------------------------------------------------*/
{
  FILE *f2;
  ...
  double maxP=35.0;
  int nf=6;
  double per[nf],f[nf],f1[22],snr[nf],norm_amp_day[nf],norm_amp_noise[nf],amp_max[nf];
  double minP=5.0, num;
...
//printf("signalmax: %f  noiserms: %f\n",signalmax,noiserms);
    ...
    norm_amp_noise[k]=snr[k]/noiserms;
...
//printf("%lf\n",noiserms*noiserms);
  sprintf(fname1,"%s_amp_l_norm_neg.txt",fname);
  f2=fopen(fname1,"w");
...
//  if(day_num<30){
  ...
  for(i=1;i<nf-1;i++) {
    fprintf(f2,"%lf %lf %lf %lf %lf %d\n",per[i],norm_amp_noise[i],norm_amp_day[i],amp_max[i],noiserms,day_num);
  }
