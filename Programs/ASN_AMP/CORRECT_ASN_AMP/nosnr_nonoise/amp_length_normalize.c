...
/*--------------------------------------------------------------*/
{
  ...
  double maxP=35.0;
  int nf=6;
  double per[nf],f[nf],f1[22],snr[nf],norm_amp[nf],amp_max[nf];
  double minP=5.0, num;
...
//printf("%lf\n",noiserms*noiserms);
  sprintf(fname1,"%s_amp_l_norm.txt",fname);
  f2=fopen(fname1,"w");
  if(snr[(int)(nf/2)]<1 || snr[nf-1-(int)(nf/2)]<1){
    fprintf(stderr,"SNR of %s less than 1. skipped!\n",fname );
    sprintf(tstr,"rm -f %s\n",fname1);
    system(tstr);
...
//  if(noiserms<0.03){
//    fprintf(stderr,"stacking time of %s is too short. skipped!\n",fname );
//    sprintf(tstr,"rm -f %s\n",fname1);
//    system(tstr);
...
//  }
  for(i=1;i<nf-1;i++) {
    fprintf(f2,"%lf %lf %lf %lf\n",per[i],norm_amp[i],amp_max[i],noiserms);
  }
