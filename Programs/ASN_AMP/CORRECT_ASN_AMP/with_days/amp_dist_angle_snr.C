...
int main (int argc, char *argv[])
{
  ...
  double stalong[5000],stalati[5000],data[5000],snr[5000],deg[5000];
  int i,ista,imax,imin;

  double PI=4*atan2(1.,1.);

  if(argc != 6)
    {
      printf("Usage: amp_dist_angle.C [center_sta_long] [center_sta_lati] [input_data_file] [deg_min] [deg_max]\n");
      exit(-1);
    }
  ...
  for (i=0;;i++) {
     if (fscanf(f,"%lf %lf %lf %lf", &stalong[i],&stalati[i],&data[i],&snr[i]) != 4) break;
    }
  ista=i;
  fclose(f);

  sprintf(outname,"%s_amp_dist_deg%.0f_to_deg%.0f",argv[3],degmin,degmax);
  ...
  for (i=0;i<ista;i++) {
     ...
     if((deg[i]<degmax && deg[i]>degmin) || (deg[i]-360<degmax && deg[i]-360>degmin) || (deg[i]+360<degmax && deg[i]+360>degmin)){
//        if(deg[i]>stadegmax){stadegmax=deg[i];imax=i;}
//        if(deg[i]<stadegmin){stadegmin=deg[i];imin=i;}
        dist=get_dist2(lati0,long0,stalati[i],stalong[i]);
        if(dist<10 || dist>1500)continue;
        fprintf(fout,"%f %f %f %f\n",dist,log(data[i]),deg[i],snr[i]);
       }
     }
  fclose(fout);
