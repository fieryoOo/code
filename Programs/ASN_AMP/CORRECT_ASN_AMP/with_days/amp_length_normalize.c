...
SAC_HD tysac;
/*--------------------------------------------------------------*/
int get_snr(char *fname,int day_num,char *comp)
/*--------------------------------------------------------------*/
{
  FILE *f2;
...
//printf("%lf\n",noiserms*noiserms);
  ...
  f2=fopen(fname1,"w");
  if(snr[(int)(nf/2)]<10 || snr[nf-1-(int)(nf/2)]<10){
    fprintf(stderr,"SNR of %s: %f. less than 10. skipped!\n",fname,snr[(int)(nf/2)] );
    sprintf(tstr,"rm -f %s\n",fname1);
    system(tstr);
...
//  if(day_num<100){
  if(noiserms<0.015){
    fprintf(stderr,"stacking time of %s: %f is too short. skipped!\n",fname,noiserms );
    sprintf(tstr,"rm -f %s\n",fname1);
    system(tstr);
  ...
  for(i=1;i<nf-1;i++) {
    fprintf(f2,"%lf %lf %lf %lf %lf %d %f\n",per[i],norm_amp_noise[i],norm_amp_day[i],amp_max[i],noiserms,day_num,snr[i]);
  }
...
//  char filename1[300];
  if(argc != 3) 
    {
      printf("Usage: amp_length_normalize [list_file_with_dayinfo] [sym pos or neg]\n");
      exit(-1);
    }
...
//    sprintf(filename1,"/utera/tianye/data_ASN_TA/%s",filename);
    get_snr(filename,day_num,argv[2]);
  }
fclose(ff);
return 1;
}
