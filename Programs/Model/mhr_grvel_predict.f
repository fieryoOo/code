	     parameter ( maxpath = 250000) 

	     character*180 map_name,pathfile_name,perlist,map_file
	     character*11 word    
	     character*1 wavetype
	     character*3 sym       
	     character*6 stnam(maxpath),evnam(maxpath)
c	     character*6 cdummy
             parameter (nmax=2000)
             real*4 per(50),grvel_R(50,maxpath),grvel_L(50,maxpath),
     1                   vel(270000)
             real*4  stalat(maxpath),stalon(maxpath),evlat(maxpath),
     1                     evlon(maxpath)
...
c	     data marg/3/,step/0.05/
             data  dlat_min/-90./,dlat_max/90.0/,dlon_min/-180.0/,
     1                dlon_max/180./
...
C----------------------------------------------------------------------------------------
	     ...
	     narg=iargc()
	     if(narg.ne.marg) STOP'USAGE: mhr_grvel_predict pathfile_name
     1           map_name  perlist'
...
C--------search for paths-----------------S
             call path_searcher(pathfile_name,ista,stalat,stalon,
     1            stnam,iev,evlat,evlon,evnam,npath)
...
C-----loop for period-----------------------------------------------------E
      	    ...
      	    do j=1,npath
       	      write(2,*) ista(j),iev(j),nper, ' ',stnam(j),' ',
     1         evnam(j), stalat(j), stalon(j), evlat(j), evlon(j)
       	      write(12,*) ista(j),iev(j),nper, ' ',stnam(j),' ',
     1         evnam(j), stalat(j), stalon(j), evlat(j), evlon(j)
...
C#####################################################################
            subroutine path_searcher(pathfile_name,ista,
     1	               stalat,stalon,stnam,iev,evlat,evlon,evnam,npath)
            character*180 pathfile_name,curr_name
            character*6 stnam(1000000),evnam(1000000)
            real*4 stalat(1000000),stalon(1000000),evlat(1000000),
     1               evlon(1000000)
	    dimension ista(1000000), iev(1000000)
            logical lf
...
4           do j=1,1000000
            read(7,*,end=999) ista(j),iev(j),stnam(j),
     1             evnam(j),stalat(j),stalon(j),evlat(j),evlon(j)
		print *, ista(j),iev(j),' ',stnam(j),
     1      evnam(j),stalat(j),stalon(j),evlat(j),evlon(j)
            enddo
999         close(7)
	    npath=j-1
