...
actable = getvalues(ptcinfodep, [2 + 4 * i for i in range(nbands)])
for i in range(len(actable)):
	actable[i] = floatlist(re.split('  | ',((actable[i]).strip())))
pctable = getvalues(ptcinfodep, [3 + 4 * i for i in range(nbands)])
for i in range(len(pctable)):
	pctable[i] = floatlist(re.split('  | ',((pctable[i]).strip())))
btable = getvalues(ptcinfodep, [1 + 4 * i for i in range(nbands)])
...
#Plots the tilt transfer functions.
tiltaplotfit(tddep, 'dep', directiondep, coherencedep, actable, btable)
tiltpplotfit(tddep, 'dep', directiondep, coherencedep, pctable, btable)
