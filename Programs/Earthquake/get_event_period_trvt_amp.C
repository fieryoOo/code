...
int main (int na,char *arg[])
{
  ...
  for(;;)
	...
	continue;
      ...
      if(event!=event_old)
	{
	  if(event_old!=0 && flag==0) fclose(f3);
          flag=0;
	  sprintf(buff1,"%ssec_%ssnr_%sdis/%ld.ph.txt",arg[1],arg[3],arg[4],event);
          ...
          if(f3=fopen(buff1,"r")) {
             fclose(f3);
             flag=1;
             cout<<buff1<<" exists, skipped."<<endl;
             continue;
          }
	  cout<<buff1<<endl;
	  f3=fopen(buff1,"w");
      ...
      if((f2=fopen(disp_filename,"r"))==NULL) continue;
      fclose(f2);
	...
	fprintf(f3,"%lf %lf %lf %lf %lf %s\n",lon,lat,time,phvel,amp,station);
    }
