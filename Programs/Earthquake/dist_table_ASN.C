...
{ 
  if(na!=3)
    {
      cout<<"usage:get_dist csta_name_lst station.lst"<<endl;
      return 0;
    }
  FILE *ff,*f1,*f2;
  char file_name[400];
  ...
  for(j=0;j<iev;j++)
    {
      for(k=0;k<ist;k++)
        {
	  sprintf(file_name,"/media/WORK/tianye/US_ASN_amp/STACK_08to11_norm/%s/COR_%s_%s.SAC",event[j],event[j],name[k]);
