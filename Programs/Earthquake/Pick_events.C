...
main(int na, char *arg[])
{
  ...
  double lon[NEV], lat[NEV], abtime[NEV], ftmp, dist;
  ...
  for(i=0;;i++) {
     ...
     strtok(info[i],"\n");
     sscanf(info[i],"%4d%2d%2d%2d%2d%2d %lf %lf %s",&yer,&mth,&day,&hor,&min,&sec,&lon[i],&lat[i],&ctlg[i]);
  ...
  for(i=0;i<nev;i++) {
     ii=0; iflag=flag[i];
     for(j=i+1;j<nev;j++) {
        if(abtime[j]-abtime[i]>200.) break;
