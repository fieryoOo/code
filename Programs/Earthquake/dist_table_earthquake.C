...
{ 
  if(na!=4)
    {
      cout<<"usage:get_dist event_lst station.lst path"<<endl;
      return 0;
    }
  FILE *ff,*f1,*f2;
  char file_name[400];
  ...
  int N=10000;
  char buff[300], event[N][20];
  char name[N][6];
  double lat[N],lon[N];
  int i,j,k,nesta,iev,ist;
  for(i=0;i<N;i++)
    {
      if(fgets(buff,300,f1)==NULL) break;
      sscanf(buff,"%s",&event[i][0]);
      //cout<<event[i]<<endl;
    }
  fclose(f1);
  ...
  for(i=0;i<N;i++)
    {
      if(fgets(buff,300,f2)==NULL) break;
      sscanf(buff,"%s %lf %lf",&name[i][0],&lon[i],&lat[i]);
      //cout<<name[i]<<endl;
    }
  fclose(f2);
  ist=i;
  ...
  for(j=0;j<iev;j++)
    {
      ...
      for(k=0;k<ist;k++)
        {
	  sprintf(file_name,"%s/%s/%s.%s.LHZ.sac",arg[3], event[j],event[j],name[k]);
      ...
      cout<<nesta<<" station extracted for event "<<event[j]<<endl;
    }
  fclose(ff);
  return 0;
}
