...
{ 
  if(na!=4)
    {
      cout<<"usage:calc_ini_ph [in_file] [period] [trvt_shift]"<<endl;
      return 0;
    }
  FILE *ff,*f1,*f2;
  ...
  double lon[N],lat[N],time[N],vel[N],amp[N],clon,clat,dtmp,temp,vell, velh;
  short YesNo[N], flag;
...
/*
  ...
  for(i=0;i<ist;i++) {
     if(YesNo[i]==1)
        fprintf(ff,"%lf %lf %lf %lf %lf %s\n",lon[i],lat[i],time[i]-6.,vel[i],amp[i], stnm[i]);
  }
  fclose(ff);
*/
  return 1;
