...
int main (int argn, char *argv[]) {
  ...
  int i,j,k,nsta,ndir,ii,jj,jjj,jjjj,flag,icent,ista1,ista2;
  char staname[NSTA][10],outname[100],dirlist[100][300],outfname[300],outdir1[300],tstr[300];
  char tname1[300],tname2[300],tname3[300],tname4[300];
  ...
  if (argn != 4) {
    fprintf (stderr,"input [station.lst with centre_sta in the 1st line] [dir.lst] [out.dir]\n");
    return 0;
    }
...
//  for (i=0;i<nsta;i++) {
     sprintf(outdir1,"%s/%s\0",argv[3],staname[icent]);
	...
	system(tstr);
     ...
     for (j=0;j<nsta;j++) {
       ...
       sprintf(outname,"COR_%s_%s.SAC",staname[ista1],staname[ista2]);
       sprintf(outfname,"%s/%s/COR_%s_%s.SAC",argv[3],staname[icent],staname[ista1],staname[ista2]);
       flag = 0;
       ...
       for (k=0;k<ndir;k++) {
        if(stat(dirlist[k],&st) != 0){
         ...
         sprintf(tname1,"%s/%s/COR_%s_%s.SAC\0",dirlist[k],staname[ista1],staname[ista1],staname[ista2]);
         sprintf(tname2,"%s/%s/COR_%s_%s.SAC\0",dirlist[k],staname[ista2],staname[ista2],staname[ista1]);
...
//	    fprintf (stderr,"yes!! file %s\n",tname4);
       ...
       if (flag==1) {
	 shd.evlo = stalon[ista1];
	 shd.evla = stalat[ista1];
	 sprintf(shd.kevnm,"%s\0",staname[ista1]);
	 shd.stlo = stalon[ista2];
	 shd.stla = stalat[ista2];
	 sprintf(shd.kstnm,"%s\0",staname[ista2]);
	 shd.npts = 6001;
	 shd.b = -3000;
