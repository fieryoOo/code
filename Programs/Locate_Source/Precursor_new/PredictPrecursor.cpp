...
int main( int argc, char* argv[] ) 
{
   ...
   if( argc != 6 ) {
      std::cerr<<"Usage: "<<argv[0]<<" [SAC list] [Input model] [source lon] [source lat] [uncertainty in group speed map (0. - 1.)]"<<std::endl;
      exit(-1);
   }
   char mapname[100]; sprintf(mapname, "%s", argv[2]);
   float slon = atof(argv[3]), slat = atof(argv[4]), unc = atof(argv[5]);
   ...
   for(ifile=filebuff.begin(); ifile<filebuff.end(); ifile++) {
      ...
      sigcur = new SACREC((*ifile).c_str(), 0., 0.);
      if( ! sigcur->load() ) {
   ...
   for(int i=0; i<filebuff.size(); i++) fout<<filebuff.at(i)<<" "<<arrtime.at(i).at(0)<<" "<<arrtime.at(i).at(1)<<std::endl;
   fout.close();
