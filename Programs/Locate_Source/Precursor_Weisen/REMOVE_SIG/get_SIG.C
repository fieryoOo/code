...
//		if (mag1 < sig[i] ) mag1 = sig[i];           // mag1 is the largest number;
 ...
 ncut = (int)t2 - (int)t1;
 ns = 7;
 if (ns < (int)(log((double)ncut)/log(2.0))) ns = (int)pow(2,(int)(log((double)ncut)/log(2.0)));
 else ns = (int)pow(2,ns);
 fprintf(stderr,"%d %d\n",ns,(int)(log((double)ncut)/log(2.0)));
 dom = 1/ns;
// abort();
 fftw_complex *out;
 out = (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * ncut);
 fftw_plan p;
 double sig2[10000];
 for (i=0;i<ncut;i++) sig2[i]=(double)sig1[i];
 p = fftw_plan_dft_r2c_1d(ncut, &(sig2[0]), out, flags);
 fftw_execute(p);
 fftw_free(out);
 abort();
