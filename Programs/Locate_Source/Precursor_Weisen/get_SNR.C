...
int main ( int argn , char *arg[] ) {
 ...
 FILE *ff,*ff1;
 char sta1[6],sta2[6],tempst[6],filename[300],stations[2000][6];
...
for (k=0;;k++) {
 if (fscanf(ff1, "%s %d %s %s", filename, &per, sta1, sta2)==EOF) break;
...
//	mag = sig[(int)dtime+(shd.npts-1)/2];
	for (i=(int)t1+(shd.npts-1)/2;i<=(int)t2+(shd.npts-1)/2;i++) {
		if (mag1 < sig[i] ) {
			mag1 = sig[i];           // mag1 is the largest number;
			flagi = i;		
			}
		 else 
			mag1 = 0;
 ...
 snr = mag1/mag2;
 width = 0;
 for (i=flagi;i<flagi+600;i++) {
	if (sig[i]> 0.5*mag1) width++;
	else break;
//	if (i>200) break;
	}
...
//        if (i<flagi-200) break;
        }


 fprintf(ff,"%s %d %s %s %g %g %g %d\n",filename, per, sta1, sta2, snr,mag1,mag2,width);
 }
}
