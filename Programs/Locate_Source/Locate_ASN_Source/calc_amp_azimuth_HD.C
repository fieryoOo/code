...
int main (int argc, char *argv[])
{
  ...
  int i, j, k, iper, jper, nfile, nsta, nost;
  float olong[5000], olati[5000], ampst[5000], longlst[5000], latilst[5000];
  float tlength[1000000], per, perl, perh, pertmp[30], amptmp[30], fstep=-0.08;
  float period, amppos, snrpos, ampneg, snrneg, longtmp, latitmp;
  double dist, alpha[5000], alphatmp, powe, amp_n[3600][2];
  double e_n=2.7182818284590451;
  if(argc != 5) 
    {
      printf("Usage: calc_amp_azimuth [sta.lst] [file_time-length.lst] [period] [time-length threshold]\n"); 
      exit(-1);
    }
  ...
  for(i=0;;i++) {
    if((fscanf(fin,"%s %f",sta_pair[i], &tlength[i]))!=2) break;
  ...
  sprintf(buff,"mkdir -p Amp_Azimuth_%ssec",argv[3]);
  system(buff);
  for(i=0;i<nsta;i++) {
//  for(i=1298;i<1299;i++) {
     ...
     nost=0;
     for(j=0;j<nfile;j++) {
        if(tlength[j]<atof(argv[4])) continue;
...
// cout<<stalst[i]<<"  "<<osta<<endl;
        ...
        ampst[nost]=(amptmp[0]/pertmp[0]+amptmp[1]/pertmp[1])/(1./pertmp[0]+1./pertmp[1])/pow(e_n,-1e-3*dist);
...
/*        if(deg_l==204) cout<<"ampsta: "<<amppos<<"  amp_h: "<<amp_n[deg_h][0]<<"  weight: "<<amp_n[deg_h][1]<<"  num: "<<amp_n[deg_h][2]<<endl;
     ...
     sprintf(outname,"Amp_Azimuth_%ssec/%s_center_amp_map",argv[3], stalst[i]);
...
//cout<<olong[j]<<"  "<<olati[j]<<"  "<<ampst[j]<<endl;
        fprintf(fout,"%6.2f %6.2f %.5g\n", olong[j], olati[j], ampst[j]*1e8);
     }
     fclose(fout);

     sprintf(buff,"minmax %s | awk '{print $5,$6}' | sed s/'<'/''/g | sed s/'>'/''/g | sed s/'\\/'/' '/g | awk '{printf \"-R%%.1f/%%.1f/%%.1f/%%.1f\\n\",$1,$2,$3,$4}' > region_temp",outname);
     system(buff);
     sprintf(buff,"/home/tianye/code/Script/GMT/C_plot_travel_positive %s region_temp 0.1", outname);
     system(buff);
     ...
     sprintf(outname,"Amp_Azimuth_%ssec/%s_amp_azimuth",argv[3],stalst[i]);
     if((fout = fopen(outname, "w"))==NULL) {
       printf("Cannot open file %s to write\n",outname);
       continue;
     }
 ...
 //       cout<<"amp: "<<amp_n[k][0]<<"  num: "<<amp_n[k][1]<<endl;
     ...
     fclose(fout);
     cout<<"The "<<i+1<<"th station "<<stalst[i]<<" completed..."<<endl;
  }
  return 1;
}
