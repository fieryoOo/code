...
c ---
      ...
      real*4    PERMAX,PERMIN,pi,R0,sc,sc_b,sdate,sqr_8pi
...
c ---
      ...
      data key_compr/.false./,fix_vel/2.85/,const1/1.E-07/,R0/6371./
C-----------ARGUMENTS-----------------------------------------------
...
c --- read requested station list ---
    ...
    4 nstai = nstai-1
      ...
      applyQ = 'Y'
      call atracer(model,elatc,elon,ncor,applyQ)
...
cMB         az(j)=sumr*ampr(j)*aM*const1*conjg(cor(j-1))
            az(j)=sumr*aM*const1*cor(j-1,2,mm)/
